...
where $\eta$ is the **learning rate**, a hyperparameter that controls the step size. As in the [cross-validation chapter](cross_validation.qmd), a hyperparameter configures the fitting procedure and is chosen by a separate validation criterion rather than by minimizing the training loss over $\boldsymbol\theta$.
...
- $\ell_i(\boldsymbol{\theta}) = \ell(y_i, \hat{y}_i(\boldsymbol{\theta}))$: loss for observation $i$. The loss function $\ell$ is the one the [cross-validation chapter](cross_validation.qmd) wrote as $L(y,\hat y)$; this chapter uses a different letter because $L$ is reserved for the average below.
...
Following the case convention fixed in the [information theory chapter](information_theory.qmd), the data $(x_i,y_i)$ enter this chapter as realized values: the algorithms condition on the training dataset throughout, and the randomness in the stochastic methods below comes from data subsampling.
...
:::{.callout-note collapse="true" title="Optional: The unnormalized momentum convention"}
