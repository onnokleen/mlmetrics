...
## Batch, Stochastic, and Mini-Batch Gradient Descent {#sec-optim-mini-batch}
...
$$
L(\theta_1,\theta_2)=\frac23\left[
(\theta_1^2+\theta_2-11)^2+(\theta_1+\theta_2^2-7)^2
+(\theta_1-4)^2(\theta_2+2)^2\right].
$$
...
So far the objects being optimized were generic objective functions. The [feed-forward neural networks chapter](feed_forward_nns.qmd) applies the same optimization ideas to networks, where gradients are computed by backpropagation.
...
2. Show that when $\delta=0$ this reduces to the gradient descent recursion of Exercise 3.1, with the single curvature $a$ here playing the role of $\lambda_{\max}$ there. For the general case, use the following Schur-Cohn stability criterion [see, e.g., @Hamilton1994TimeSeries, sec. 1.2]: a second-order linear recursion $x_t=cx_{t-1}+dx_{t-2}$ with characteristic polynomial $p(r)=r^2-cr-d$ converges to zero from every initial state if and only if $p(1)>0$, $p(-1)>0$, and $|d|<1$. Apply this to show that, for general $\delta\in[0,1)$, the recursion converges to zero from every initial state $(\theta^{(0)},u_0)$ if and only if
   ...
   $$
3. Write $c=1+\delta-\eta(1-\delta)a$. Show that the characteristic roots are complex conjugates if and only if $c^2<4\delta$, and that both then have modulus $\sqrt\delta$. Compare momentum with $\delta=1/4$ and vanilla gradient descent at the same learning rate $\eta=1/a$: compute the characteristic roots for momentum and the iterate after one gradient descent update. Explain why the wider stability interval for momentum does not by itself imply faster convergence.
...
$$
r_{\pm}=\frac{c\pm\sqrt{c^2-4\delta}}{2}.
$$
They are complex conjugates exactly when $c^2<4\delta$. In that case their product is $r_+r_-=\delta$ and $r_-=\overline{r_+}$, so $|r_+|=|r_-|=\sqrt\delta<1$. The resulting real iterates oscillate with an envelope proportional to $(\sqrt\delta)^t$.

At $\eta=1/a$ and $\delta=1/4$, $c=1/2$ and
$$
r_{\pm}=\frac14\pm\frac{\sqrt3}{4}\,\mathrm{i},
\qquad |r_{\pm}|=\frac12,
$$
where $\mathrm{i}^2=-1$. Momentum therefore generally approaches zero through damped oscillations. At the same learning rate, vanilla gradient descent gives
$$
\theta^{(1)}=(1-\eta a)\theta^{(0)}=0
$$
from every starting value and stays there. Momentum's upper stability bound is larger than $2/a$ when $\delta>0$, but that only describes which step sizes converge. Within the stable region, the characteristic roots determine the rate, and the wider interval alone does not guarantee faster convergence.
:::
