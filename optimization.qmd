...
The algorithms stop after at most 100 iterations. The stopping criterion at the end of each iteration is whether $l(x,y)<1.01$, which is close to the global minimum value of 1. Most optimizers use a learning rate of 0.3; the momentum panel uses a larger learning rate of 0.5 because the convex-combination velocity $v_t = \delta v_{t-1} + (1-\delta)g_t$ already shrinks the effective step by a factor of roughly $1-\delta$. More details can be found if you unfold the code via "Show the code". Note that the SGD optimization is illustrated by adding mini-batch-style noise to the gradient update. In this 2D example, the noise is artificial because there is no actual dataset behind the contour.
