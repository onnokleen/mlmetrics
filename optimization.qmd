...
- $\ell_i(\boldsymbol{\theta}) = \ell(y_i, \hat{y}_i)$: loss for observation $i$
...
Three remarks connect this notation to the previous chapter. First, [the cross-validation chapter](cross_validation.qmd) wrote the per-observation loss as $L(y,\hat y)$ and called its sample average the empirical risk. We now write $\ell_i(\boldsymbol{\theta})$ for the per-observation loss viewed as a function of the parameters and $L(\boldsymbol{\theta})$ for its average—the same objects in parameter-dependent form. Second, $L(\boldsymbol{\theta})$ is the *empirical* training loss: optimization in this chapter minimizes it, while the generalization risk $R$ of that chapter is what validation ultimately judges. The tendency of training loss to understate generalization risk is the overfitting concern that motivated cross-validation. Third, in the sense of the case convention fixed in the [information theory chapter](information_theory.qmd), the data $(x_i,y_i)$ enter this chapter as realized values: the algorithms condition on the training dataset throughout, and the randomness in the stochastic methods below comes from drawing mini-batches.
...
Adam is widely used for neural networks [@KingmaBa2015]. It combines the ideas of **Momentum** (using the first moment, or mean, of the gradients) and **RMSprop** (using the uncentered second moment of the gradients).
...
Having derived the update rules one by one, we now place their computational tradeoffs side by side. The column **Optimizer state** reports the number of additional state scalars stored per scalar model parameter, equivalently the number of parameter-shaped state buffers stored alongside the parameter vector. It reflects the memory overhead of the update rule and should not be confused with dataset memory, which is controlled separately by the batch size. The citations in the algorithm column identify original or standard sources for the underlying methods; as noted above, the normalized Momentum convention used in this chapter differs from Polyak's original heavy-ball recursion. The remaining columns give criterion-specific comparisons that are qualified immediately below the table.
...
$$
which is a quadratic with uneven curvature across directions, as in Exercise 3.1. Its Hessian eigenvalues are $0.2$ and $2$, so its condition number is $10$. The two coordinates are interchanged relative to that exercise: here the flat direction is $\theta_1$ and the steep one is $\theta_2$, whereas Exercise 3.1 places the larger eigenvalue $\lambda_{\max}$ on $\theta_1$.
...
#| fig-cap: "Paths of six update rules on the anisotropic quadratic $L(\\theta_1,\\theta_2)=1+0.1\\,\\theta_1^2+\\theta_2^2$, whose Hessian condition number is 10. In each panel the red line traces the parameter path from the common starting point (green dot) to the final iterate (red dot); the black star marks the global optimum at the origin, gray contour lines show loss levels, and the panel title reports the number of steps taken. The noisy-gradient panel adds artificial Gaussian noise to the exact gradient rather than sampling observations from an empirical loss."
#| code-fold: true
import numpy as np
