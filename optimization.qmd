...
:::{.callout-note}
## Exercise 4.3: Diagnosing Learning Curves

For each of the following training scenarios, choose:

- the most likely diagnosis from `{underfitting / optimization too slow, overfitting, learning rate too high}`
- the most appropriate first response from `{increase learning rate, add regularization or early stopping, reduce learning rate}`

Assume the model is trained with mini-batch gradient descent and performance is monitored on a validation set.

1. The training loss and validation loss are both high, very close to each other, and both decrease only very slowly over many epochs.
2. The training loss continues to fall, but the validation loss starts rising after an initial decline.
3. Both the training loss and validation loss oscillate strongly from epoch to epoch and do not show a stable downward trend.

*Exam level. The exercise turns learning-curve interpretation into a bounded diagnosis problem with definite answers.*
:::

:::{.callout-warning collapse="true"}
## Hint for Part 1
A small train-validation gap usually points away from overfitting.
:::

:::{.callout-warning collapse="true"}
## Hint for Part 2
Here the optimization on the training sample is still improving, but out-of-sample performance is deteriorating.
:::

:::{.callout-warning collapse="true"}
## Hint for Part 3
Persistent oscillation is usually a sign that the optimization steps are too aggressive rather than too cautious.
:::
...
Diagnosis: **underfitting / optimization too slow**.  
First response: **increase learning rate**.

Reason: both losses are high and close, so the model is not primarily suffering from overfitting. The main issue is that optimization is progressing too slowly.

**Part 2**

Diagnosis: **overfitting**.  
First response: **add regularization or early stopping**.

Reason: training performance keeps improving, but validation performance deteriorates. That is the standard pattern of overfitting.

**Part 3**

Diagnosis: **learning rate too high**.  
First response: **reduce learning rate**.

Reason: strong oscillation in both training and validation losses indicates unstable optimization steps that overshoot the local descent direction.
:::
