...
-   **Cons**: It is computationally very expensive and memory-intensive, especially for large datasets. It is infeasible to load millions of samples into memory at once. Can we prone to local optima but this depends heavily on the learning rate.
:::

### Stochastic Gradient Descent (SGD) 

Instead of In Stochastic Gradient Descent, we calculate the gradient and update the parameters using just **one training sample** at a time. The gradient approximation reads as follows:
...
:::{.callout-tip}
## Definition: Epoch
An epoch is referring to the instances for which we once went through all data points $i$ for optimization. With the notion of epoch introduced, we get the following quasi code for neural network optimization:
...
For that, let's define $\boldsymbol G_t = \sum_{\tau = 1}^t \boldsymbol g_\tau \boldsymbol g_\tau^\top + \varepsilon \boldsymbol I_m$ the outer-product of gradients with small adjustment $\varepsilon > 0$ and $\boldsymbol I_t$ an identity matrix with dimension equal to the length of $\boldsymbol \theta$. 
...
Here it becomes clear why we needed to add the small $\epsilon$ component: It is small smoothing constant to prevent division by zero.
...
Notice how SGD is noisier than Batch Gradient Descent, Momentum overshoots but corrects, AdaGrad stalls, RMSProp corrects the stalling, and Adam looks like a combinations of momentum and RSMprop.
...
:::{.callout-tip}
## Essential Concepts for Machine Learning Practitioners

**1. Gradient Descent is the Foundation**

- All modern optimizers are variations of the basic gradient descent idea: move in the direction of steepest descent
- The core challenge is estimating gradients efficiently and choosing appropriate step sizes

**2. Mini-Batch is the Standard**

- Pure batch gradient descent is too expensive for large datasets
- Pure SGD is too noisy and unstable
- Mini-batch gradient descent (typically 32-128 samples) provides the best balance

**3. Adaptive Learning Rates are Crucial**

- Fixed learning rates are hard to tune and often suboptimal
- AdaGrad, RMSprop, and Adam automatically adjust learning rates per parameter
- This is especially important for high-dimensional problems with different parameter scales

**4. Adam is Usually a Good Default**

- Combines momentum (first moment) with adaptive learning rates (second moment)
- Robust across many different types of problems
- Default hyperparameters work well in practice

**5. Algorithm Choice Matters More for Complex Landscapes**

- On simple, convex problems, most optimizers perform similarly
- On complex neural network loss surfaces, adaptive methods show clear advantages
- SGD's noise can sometimes help escape local minima, but adaptive methods are generally more reliable
:::
