...
:::{.callout-note title="Algorithm: Mini-Batch Training by Epoch"}
...
The version commonly used in software (and called *AdaGrad* below) retains only coordinate-wise squared gradients, a form of diagonal preconditioning. Define the elementwise gradient accumulator
$$
...
| **SGD** [@RobbinsMonro1951] | 0 | $\eta$ | Low computation and memory per update | High gradient noise; sensitive to $\eta$ |
...
| **Momentum** (normalized convention; idea from @Polyak1964) | 1 (velocity) | $\eta$, $\delta$ | Dampens alternating-gradient oscillation and can widen the stable learning-rate range | Can overshoot; requires tuning |
...
This exercise uses the equivalent $t\mapsto t+1$ indexing convention to make the scalar recursions easier to read; the main text labels the same update by its destination index $t$.
