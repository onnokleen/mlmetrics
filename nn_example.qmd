...
The empirical comparison has two purposes. First, it shows why modeling $\log(RV)$ is often more stable than modeling realized variance in levels. Second, it compares a linear HAR-OLS benchmark with four neural-network specifications — a parsimonious HAR-FNN, a full feed-forward network, a Simple RNN, and an LSTM — all evaluated under the same forecast-origin discipline. A central finding is that HAR-OLS is hard to beat: the nonlinear HAR-FNN loses to its linear counterpart on every loss, and the full-history neural networks at best match HAR-OLS.
...
The workflow below makes the econometric design explicit: it constructs forecast origins before splitting the sample, estimates scaling constants only on the training block, and keeps the test block untouched until the final comparison. Concretely, features and target are standardized to zero mean and unit variance using the training-block mean and standard deviation (cf. [the Cross-Validation chapter on fitting preprocessing on training data only](cross_validation.qmd#sec-cv-leakage)), and forecasts are mapped back to the original scale. Replication uses the local SPY realized-variance file documented in [the dataset appendix](datasets.qmd#sec-spy-taq-realized).
...
\right).
$$
The training block is the first 64% of valid forecast origins, the validation block is the next 16%, and the test block is the final 20%. The validation block is sized to give enough forecast origins for stable hyperparameter selection via Optuna while keeping the test block large enough for a reliable out-of-sample comparison. Optuna is a hyperparameter-search library: a *trial* is one sampled hyperparameter configuration, evaluated on the validation objective, and its default sampler is TPE, a tree-structured Bayesian search strategy. See the [chapter on hyperparameter optimization](hpo.qmd) for the underlying selection-bias and search-strategy issues.
...
- The **HAR-FNN** is the most structured specification. It uses the three HAR volatility-component summaries — daily, weekly, and monthly averages of realized variance — that follow the @CorsiHAR2009 model.
...
@fig-nn-architectures summarizes the neural specifications. The models differ in input representation, not in parameter count, and the information available at forecast origin $t$ is held fixed and documented across architectures. HAR-OLS is included because the nonlinear HAR-FNN is informative only as a comparison against the linear HAR model it generalizes.
...
Each architecture below uses the same skeleton — one hidden layer of tuned width followed by a linear output. The input shapes enforce the intended information sets: three HAR features for the HAR-FNN, 22 flattened lags for the full FNN, and a 22-step sequence for the recurrent models.
...
In the local run that produced this chapter's output, chronological validation selected the following hyperparameters. Validation losses are comparable within a target scale but not across the `log(rv)` and `rv` panels, because the training targets have different units and variances.
...
The columns answer different questions. Here RMSE $=\sqrt{\frac{1}{n}\sum_t(\hat z_t-z_t)^2}$ is the square root of the mean squared error of the forecasts. RMSE on $\log(RV)$ evaluates forecasts on the scale used by the log-target models and is the main comparison criterion here. For level-target models, the log-scale metrics are computed after clipping nonpositive variance forecasts to a small positive number before taking logs. RMSE on $RV$ evaluates the implied variance forecasts after exponentiating log-target forecasts. This back-transformation carries a subtlety: if $\hat z_t$ targets the conditional *mean* of $\log RV_t$, then $\exp(\hat z_t)$ estimates the conditional *median* of $RV_t$, not its conditional mean. A lognormal-type correction of the form $\exp(\hat z_t + \tfrac{1}{2}\hat\sigma^2)$ would be required for a mean forecast (this is the identity used in the [distributional networks chapter](distributions_via_nns.qmd) to map log-scale Gaussian forecasts to the variance scale). Because RV-scale squared-error and QLIKE losses are minimized by conditional-mean forecasts, the RV-scale columns compare a median-type functional (log-target models, uncorrected) against a mean-type functional (level-target models); read them with that handicap in mind. The log-scale RMSE column, i.e. the main criterion here, is unaffected. QLIKE is the volatility loss
$$
...
@fig-nn-performance makes the transformation choice visible. On the common log-realized-variance scale, the LSTM is the best-performing model, but its RMSE advantage over HAR-OLS is 0.0003 log-RV units — a difference that is economically negligible. HAR-OLS essentially matches the full FNN and Simple RNN as well. That is the main benchmark lesson: the neural networks are not automatically better just because they are more flexible. The level-target rows are well-behaved in this updated run, but they generally lose accuracy on the log-realized-variance scale.

Two notes on reading these rankings. First, the single test window contains a few hundred forecast origins, so the standard error of any per-model loss average is non-negligible: differences smaller than the standard deviation of the per-period squared-error differences should be read as ties. The 0.0003 log-RV-unit LSTM-versus-HAR-OLS gap is well within that band. A @DieboldMariano1995 test would formalize this, with a HAC variance under serial dependence in the loss differential; we omit it here to keep the chapter compact, but readers replicating the experiment should not interpret RMSE differences below roughly 0.01 as robust to test-window resampling. Second, the QLIKE column in the table above is the @Patton2011 quasi-likelihood loss for volatility, which is the standard convex robust loss in the high-frequency volatility-forecasting literature.
...
4. Adding a nonlinear hidden layer to HAR inputs (HAR-FNN) does not improve on linear HAR-OLS in this sample; the binding constraint is the choice of input features, not linearity.
