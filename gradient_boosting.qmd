...
Classification boosting therefore retains the residual structure, but the residual is on the probability scale ($y_i \in \{0,1\}$ minus $p_i \in (0,1)$) rather than on the outcome scale.

This is exactly a case where the leaf-value refinement above matters: the tree fit to the pseudo-residuals $r_{im} = y_i - p_i$ fixes only the splits, and because Bernoulli log-loss is not squared error, the leaf constants are then set by the per-leaf line search $\gamma_{jm}$ rather than by the mean residual in the leaf. That minimization has no closed form here, so implementations take a single Newton step, $\gamma_{jm} = \big(\sum_{i:\, x_i \in R_{jm}} (y_i - p_i)\big) \big/ \big(\sum_{i:\, x_i \in R_{jm}} p_i(1-p_i)\big)$, using the gradient $p_i - y_i$ and the curvature $p_i(1-p_i)$ evaluated at the current probabilities.

:::{.callout-important}
## Connection to Cross-Entropy

For classification, boosting with Bernoulli log-loss is minimizing cross-entropy, i.e. the negative log-likelihood of a Bernoulli model. This links boosting directly to the [MLE-as-KL discussion](information_theory.qmd#sec-mle-kl-divergence): fitting the boosted classifier means reducing the discrepancy between observed class outcomes and predicted event probabilities.
:::
...
This is called **shrinkage**. The interaction between $\nu$ and the optimal number of trees $M$ is roughly reciprocal: a smaller $\nu$ requires more iterations to reach the same training risk, so halving $\nu$ approximately doubles the optimal $M$ chosen by validation. This reciprocal relation is an empirical regularity reported by @Friedman2001Greedy, not a theorem. What @ZhangYu2005 establish is a different and more basic point: boosting stopped early is consistent, which is what justifies treating the iteration count as a regularization device in the first place. The product $\nu \cdot M$ behaves like a complexity budget, which is the sense in which shrinkage acts as a regularization device.
...
See the scikit-learn example [Monotonic Constraints](https://scikit-learn.org/stable/auto_examples/ensemble/plot_monotonic_constraints.html) for a visual comparison of constrained and unconstrained fits and a full working API example.

:::{.callout-note title="Implementation Note: XGBoost"}

XGBoost [@ChenGuestrin2016] is an optimized implementation family for gradient-boosted trees. Relative to the basic algorithm in this chapter, it adds engineering and regularization features such as efficient split search, L1/L2-style penalties, sparse or missing-value handling, monotone constraints, and scalable training. The econometric interpretation remains the same: it is still a tuned predictive tree ensemble, so validation design and the information set are central.
:::
...
The same qualitative pattern appears on actual macroeconomic data. @fig-gb-fredqd shows training and validation MSE curves for annualized GDP growth in quarter $t+1$ from FRED-QD, using five predictors dated quarter $t$ — GDP growth, inflation, the term spread, log VIX, and the unemployment rate — and a time-ordered 75/25 split.[^gb-fredqd-final-vintage] With a small learning rate, the validation curve declines gradually and remains relatively flat; with a large learning rate, validation error reaches its minimum early and then begins to rise — the overfitting signal that early stopping is designed to detect. The absolute MSE values are large relative to the synthetic example because quarterly GDP growth is genuinely difficult to forecast: the 2020 contraction alone generates residuals of tens of annualized percentage points, which dominate the average squared error.
...
```{python}
#| label: fig-gb-fredqd
#| fig-cap: "Training and validation MSE curves for gradient boosting on one-step-ahead GDP growth (annualized log-difference of GDPC1) from FRED-QD, with a time-ordered 75/25 split. Predictors are lagged GDP growth, inflation, the term spread (10y minus 3m Treasury yield), log VIX, and the unemployment rate. Left panel: learning rate 0.02; right panel: learning rate 0.2. Each model uses 250 trees with max depth 2. The high absolute MSE values reflect the difficulty of GDP growth forecasting: the 2020 contraction produces residuals of tens of annualized percentage points that dominate the average. The qualitative pattern — flatter validation curve under the small learning rate, earlier overfitting under the large one — is the key lesson."
#| echo: true
...
for (lr, _), ax in zip(configs, axes):
    ...
    ax.plot(train_mse, color="C0", linewidth=2, label="Training MSE")
    ax.plot(val_mse,   color="C3", linewidth=2, label="Validation MSE")
    ax.set_title(f"Learning rate = {lr}")
    ax.set_xlabel("Number of trees")
    ax.grid(True, alpha=0.3)
    ax.legend(frameon=False)
