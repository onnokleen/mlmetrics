...
:::{.callout-note collapse="true" title="More info: Pinball and Huber losses"}
...
Its negative loss gradient with respect to $F(x_i)$ is the clipped residual $\max\{-\delta,\min(u_i,\delta)\}$. Large forecast errors therefore cannot generate arbitrarily large pseudo-residuals. The general lesson is that gradient boosting is not tied to squared, Bernoulli, or exponential loss: the chosen loss determines the population target, the pseudo-residuals, and the loss-optimal leaf values.
:::
...
after which the update becomes $\hat F_m = \hat F_{m-1} + \nu \sum_j \gamma_{jm} \mathbf{1}\{x \in R_{jm}\}$ rather than $\hat F_{m-1} + \nu \hat h_m$. For the squared-error case the two coincide — the minimizing $\gamma_{jm}$ is just the mean pseudo-residual in the leaf — so the distinction only matters for general losses such as the Bernoulli loss introduced next. This per-leaf line search is Friedman's [@Friedman2001Greedy] TreeBoost refinement.
...
Extreme gradient boosting (XGBoost) [@ChenGuestrin2016] is an optimized implementation family for gradient-boosted trees. Relative to the basic algorithm in this chapter, it adds engineering and regularization features such as efficient split search, $\ell_1/\ell_2$ penalties, sparse or missing-value handling, monotone constraints, and scalable training. The econometric interpretation remains the same: it is still a tuned predictive tree ensemble, so validation design and the information set are central.
:::
...
6. Now suppose the observations are monthly and ordered in time, and the number of boosting iterations $M$ is selected from a large grid. Explain why random $K$-fold validation can select an overly large $M$ for a forecasting application, and describe a validation design that targets the intended one-step-ahead forecasting problem.

*Exam level. Parts 1--5 combine boosting mechanics, shrinkage, and the regularization interpretation of the learning rate; Part 6 diagnoses how validation design interacts with the iteration count.*
:::
...
Ask whether every training fold would have been available before the observations in its validation fold at a real forecast origin.
:::
...
Random $K$-fold validation mixes earlier and later months. A training fold can therefore contain observations dated after a validation observation. If the relationship drifts or the predictors and outcomes are serially dependent, this comparison does not reproduce the information set of a one-step-ahead forecaster. Because larger $M$ gives the ensemble more flexibility to exploit sample-specific patterns, the contaminated validation design can make a long boosting path look better than it will at genuine future origins and can select an overly large $M$.

Use rolling- or expanding-window validation instead. At every validation origin, fit the full preprocessing and boosting procedure only on observations available before that origin, evaluate the one-step-ahead loss, and choose $M$ from the average loss across those ordered origins. This treats the iteration count as a hyperparameter and estimates it under the same timing protocol as deployment.
:::
