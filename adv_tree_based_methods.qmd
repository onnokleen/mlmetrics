...
#| fig-cap: "QRF forecasts for one-step-ahead GDP growth (annualized log-difference of GDPC1) over the final 25% of FRED-QD quarters. The horizontal axis gives the target quarter. The blue band spans the conditional 10th to 90th percentile, the solid blue line is the conditional median, and the gray line is realized GDP growth. Conditional quantiles use original-observation neighborhood weights from 200 trees with a minimum in-bag leaf size of five."
#| echo: true
...
# For each test point, accumulate normalised weights across trees
...
The interval width varies across the test period because the QRF neighborhood changes with the predictors; a homoskedastic parametric model would instead impose a constant width around its conditional mean. The pandemic quarters expose the more important limitation. Realized growth reaches roughly $-33$ and then $+30$ annualized percent in 2020, while the forecast band remains far inside those values. By construction, $\hat F(y \mid X=x)$ reweights outcomes observed in the training sample and therefore cannot place mass beyond their range. When the training neighborhood contains no comparable episode, changing the quantile level cannot recover the missing tail support.
...
$$
so every training observation initially receives the same predictive distribution. The boosted stages then let its parameters vary with $x$.
...
@fig-ngboost-natural-step makes the rescaling visible for a Gaussian negative log-likelihood parameterized by its mean $\mu$ and log standard deviation $\ell=\log\sigma$. The contour lines are equal-loss sets for a five-observation sample. Both arrows start from the same parameter value and use the same nominal step size; they differ only in whether the loss gradient is used directly or premultiplied by inverse Fisher information.

```{python}
#| label: fig-ngboost-natural-step
#| fig-cap: 'Ordinary and natural-gradient descent directions for a Gaussian negative log-likelihood in the parameterization $(\mu,\ell)$, where $\ell=\log\sigma$. Contours show the sample-average loss above its minimum. Both arrows start at $(\mu,\ell)=(-1,0.4)$ and use the same nominal step size. Fisher scaling places relatively more weight on correcting the mean at this starting point, so the natural-gradient arrow moves farther horizontally and less vertically.'
#| echo: true
...
ell_grid = np.linspace(-1.0, 1.2, 240)
MU, ELL = np.meshgrid(mu_grid, ell_grid)
LOSS = ELL + 0.5 * np.mean(
...
ordinary = np.array([
    np.exp(-2.0 * ell0) * (mu0 - y_demo.mean()),
    1.0 - np.exp(-2.0 * ell0) * mean_sq_resid,
])
fisher = np.diag([np.exp(-2.0 * ell0), 2.0])
natural = np.linalg.solve(fisher, ordinary)
step = 0.8

fig, ax = plt.subplots(figsize=(8.2, 4.8))
levels = [0.02, 0.05, 0.10, 0.20, 0.40, 0.80, 1.50, 2.50, 4.00]
contours = ax.contour(MU, ELL, LOSS, levels=levels, cmap="Greys")
ax.clabel(contours, inline=True, fontsize=8, fmt="%.2g")
ax.scatter([mu0], [ell0], color="black", s=45, zorder=4, label="Starting value")
...
In @fig-ngboost-natural-step, the ordinary direction reacts to the numerical coordinates themselves, whereas the natural direction measures a change relative to how much it moves the predictive distribution. The figure is illustrative rather than a convergence comparison: line search and repeated boosted stages still determine the realized path.

The Bernoulli leaf update in the [gradient-boosting chapter](gradient_boosting.qmd#sec-gb-general) is the scalar version of this connection. Its Newton denominator $\sum_i p_i(1-p_i)$ is the Fisher curvature of the leaf's log-odds parameter, so dividing the score by that quantity is a one-parameter Fisher-scoring step. NGBoost extends that information scaling to a vector of distribution parameters and then approximates each component with a base learner.
...
\qquad m=1,\ldots,M.
$$
This update is analogous to the stagewise descent of the previous chapter, now along a vector of distribution parameters. There is one important difference in the line search: $\rho^{(m)}$ is a single scalar for the whole multi-parameter stage, whereas $\gamma_{jm}$ in the [gradient-boosting chapter](gradient_boosting.qmd) is chosen separately for each leaf. The learning rate $\nu$, the number of iterations $M$, and the need for time-aware early stopping carry over. Fisher scaling makes the direction invariant to smooth one-to-one reparameterizations and prevents the numerical units of one parameter from dominating the update [@DuanAvatiDingThaiBasuNgSchuler2020].
...
- use a softplus transformation $\zeta(x)=\log(1+e^{x})$, which is smooth and strictly positive, for positive scale parameters
...
|---|---|---|---|
| QRF | Weighted empirical CDF from forest neighbors | Nonparametric shape and adaptive intervals | Few local tail observations and no extrapolation beyond observed outcomes |
...
3. QRF can produce conditional quantiles and prediction intervals without specifying a parametric distribution.
4. NGBoost learns parameters of a predictive distribution by minimizing a proper scoring rule such as NLL.
5. Under NLL, natural gradients rescale ordinary gradients by inverse Fisher information, making the descent direction insensitive to smooth reparameterization.
...
*Exam level. Part 1 connects pooled neighbors to forest weights and weighted empirical quantiles, Part 2 checks the neighborhood interpretation, and Part 3 adds an econometric diagnostic about tail calibration.*
:::
...
4. Let $\theta(x)=\log \lambda(x)$, so $\lambda=e^\theta$. Derive $\partial \ell/\partial \theta$, $\mathcal{I}_{\theta\theta}$, and the natural gradient in the $\theta$ parameterization. Show that mapping this direction back to the $\lambda$ scale gives the natural-gradient direction from Part 2, and explain how the log-rate parameterization enforces the parameter constraint.
...
*Exam level. The exercise connects model comparison, likelihood calculation, reparameterization, and a testable restriction of the exponential family.*
:::
...
\tilde g_\lambda,
$$
which is the direction obtained in Part 2. This agreement illustrates the reparameterization invariance of the natural gradient. The log-rate parameterization also turns the constrained rate $\lambda>0$ into an unconstrained parameter $\theta\in\mathbb{R}$, while $\lambda=e^\theta$ guarantees a positive implied rate after every update.

**Part 5: Exponential-family misspecification**
