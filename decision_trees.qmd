...
A sufficiently deep tree can often interpolate the training sample by creating singleton or outcome-homogeneous leaves. Zero training error is not guaranteed, however: observations with identical predictor vectors but different outcomes cannot be separated by axis-aligned splits, and minimum-leaf-size or other split constraints may also prevent isolation. A close fit to training outcomes need not translate into accurate predictions for new observations.

For regression trees, additional splits can capture finer features of the conditional mean, but smaller leaves estimate their means from fewer observations. The partition itself also depends on the training sample: changing a split near the top changes the observations available to every split below it. Together, these mechanisms can make predictions from deep trees sensitive to the training sample. Pruning collapses branches, trading a coarser approximation for potentially more stable predictions.

:::{.callout-important}
## Bias-Variance View

Fix a predictor value $x_0$ and draw a new response $Y_0$ from the conditional distribution at that point, independently of the training sample. Write $f(x_0)=\mathbb{E}[Y_0]$ and $\sigma^2(x_0)=\operatorname{Var}(Y_0)$. With finite second moments, expected squared prediction error decomposes as
$$
...
+\underbrace{\sigma^2(x_0)}_{\text{irreducible noise}}.
$$
The expectation on the left averages over both new responses and repeated training samples. Estimation variance measures how much the prediction at $x_0$ changes across training samples; bias is the difference between its average and $f(x_0)$ and reflects the entire fitting procedure, including restrictions on tree size.

Pruning can improve prediction when the reduction in variance outweighs the increase in squared bias. The decomposition does not guarantee that every pruning step helps.

For time series, a later observation is not automatically independent of the training sample; the technical note below explains the relevant condition.
:::

:::{.callout-warning collapse="true" title="Technical Note: Dependent Data"}

In the independent-draw setup, the mean-zero noise $Y_0-f(x_0)$ is orthogonal to the fitted prediction, so their cross-term vanishes. For time series, use the forecast-origin information set $\mathcal{F}_t$, containing what is available at time $t$, as in the [Cross-Validation chapter](cross_validation.qmd#sec-cv-time-series). Define the future innovation as $\varepsilon_{t+1}=Y_{t+1}-\mathbb{E}[Y_{t+1}\mid\mathcal{F}_t]$. A forecast $\hat y_{t+1\mid t}$ issued at $t$ for $Y_{t+1}$ must be $\mathcal{F}_t$-measurable. With finite second moments,
$$
\mathbb{E}[\varepsilon_{t+1}\hat y_{t+1\mid t}\mid\mathcal{F}_t]
=\hat y_{t+1\mid t}\,\mathbb{E}[\varepsilon_{t+1}\mid\mathcal{F}_t]
=0.
$$
This conditional orthogonality supplies the corresponding cross-term cancellation in a forecast-error decomposition; independence of successive observations is unnecessary.
:::
