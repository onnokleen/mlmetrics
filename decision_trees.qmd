...
Decision trees are nonparametric estimators that approximate an unknown conditional mean or conditional event probability by repeatedly splitting the predictor space and assigning a constant prediction within each resulting region. The classical reference is the Classification and Regreesion Tree (CART) monograph of @BreimanFriedmanOlshenStone1984. For econometricians, trees are useful when thresholds, interactions, and regime changes matter but are hard to specify in advance. A tree can learn rules such as "if the output gap is negative and credit spreads are high, predict lower GDP growth" without the researcher having to write down all interactions beforehand.
...
@fig-dt-fredqd gives a concrete illustration using FRED-QD data. The target is annualized GDP growth in quarter $t+1$, while the predictors are macro and financial variables dated quarter $t$.[^dt-fredqd-final-vintage] In this fit, a depth-3 regression tree splits first on the term spread — a well-known leading indicator of business cycle turning points [@EstrellaMishkin1998] — then refines using unemployment and log VIX. The piecewise-constant structure is visible in the leaf values: regardless of how far predictors move during the test period, the tree can only return one of eight constants. This is a direct manifestation of the extrapolation limitation.
