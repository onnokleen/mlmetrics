...
@fig-dt-fredqd gives a concrete illustration using FRED-QD data. The target is annualized GDP growth in quarter $t+1$, while the predictors are macro and financial variables dated quarter $t$.[^dt-fredqd-final-vintage] In this fit, a depth-3 regression tree splits first on the term spread — a well-known leading indicator of business cycle turning points — then refines using unemployment and log VIX. The piecewise-constant structure is visible in the leaf values: regardless of how far predictors move during the test period, the tree can only return one of eight constants. This is a direct manifestation of the extrapolation limitation.
