...
One could also use misclassification error, $\min\{\hat p, 1-\hat p\}$, but it is flatter around the center and therefore less sensitive to changes in node composition. For that reason, entropy and Gini impurity are commonly used for growing trees, while misclassification error can be used to assess a grown classification tree [@BreimanFriedmanOlshenStone1984].
...
5. Now suppose the observations are quarterly and $\alpha$ must be selected before forecasting the final year. Explain why randomly shuffling quarters into validation folds can leak future information. Describe a valid expanding-window comparison of two candidate values, $\alpha_1$ and $\alpha_2$, and state which observations may be used to fit each forecast.

*Exam level. Parts 1--3 test tree fitting and pruning, Part 4 checks extrapolation, and Part 5 tests honest time-series tuning.*
:::
...
**Part 5: Time-aware tuning**

Random folds can train a tree on quarters that occur after a validation quarter. The resulting error no longer measures a forecast based only on the information available at that origin and can make either candidate $\alpha$ look artificially accurate.

An expanding-window comparison chooses several validation origins $t_1<\cdots<t_K$ before the final test year. At each $t_k$, fit one tree with $\alpha_1$ and one with $\alpha_2$ using only observations dated at or before $t_k$, then score their forecasts for $t_k+1$. Average the validation losses across origins and choose the candidate with the lower loss. Neither the outcome at $t_k+1$ nor any later observation may enter the fit that produces the forecast for $t_k+1$.
:::
