...
Single trees are interpretable, but that interpretability comes with three weaknesses: trees are discontinuous, unstable, and unable to extrapolate outside the support of the training data. These motivate the move to random forests and boosting in the next chapters.
...
There is a closer classical relative. A *regressogram* — the oldest nonparametric regression estimator — partitions the predictor space into fixed bins chosen in advance and reports the sample mean of $y$ within each bin. Equation (10.1) is exactly that estimator: a weighted sum of bin indicators with the leaf means as coefficients. The single difference, and the entire contribution of the tree algorithm, is that the bins are not fixed in advance but *chosen adaptively from the data*, with both the splitting variable and the threshold selected to reduce within-bin heterogeneity. That difference is what buys the tree its ability to find thresholds the researcher did not specify, and it is also the source of every statistical complication in this chapter: because the partition is estimated, leaf means are not simple averages over pre-specified cells, and standard errors computed as if the bins were fixed would be wrong.
:::
...
After the best split is chosen, the same procedure is applied within each child node. The recursion needs a termination rule, and the default conventions are worth stating: a node is not split further when it is *pure* (all $y_i$ in the node are equal, so no split can reduce the criterion), when it contains fewer observations than a minimum node size, or when no candidate split reduces the criterion. The pre-pruning controls discussed later in this chapter (maximum depth, minimum leaf size) tighten these base rules. This top-down greedy algorithm is called **recursive binary splitting**. It is computationally feasible, but each split is only myopically optimal given the splits already made — the resulting tree need not minimize the sum of squared errors among all trees with the same number of leaves.
...
#| fig-cap: "A depth-2 regression tree over the output gap (horizontal axis) and credit spread (vertical axis). Background colors indicate fitted leaf values, point colors indicate observed responses, and the vertical and horizontal boundaries are the tree's split thresholds."
#| echo: true
...
#| fig-cap: "Entropy (blue) and Gini impurity (orange) as functions of the leaf event probability on the horizontal axis."
#| echo: true
...
One could also use misclassification error, $\min\{\hat p, 1-\hat p\}$, but it is flatter around the center and therefore less sensitive to changes in node composition. For that reason, entropy or Gini are more common for tree growing; misclassification error is more commonly used for pruning the grown tree, where exact ranking is less critical.
...
A fully grown tree can drive training error arbitrarily low by creating many tiny leaves; that flexibility is dangerous because the resulting estimator has low bias but high variance.

:::{.callout-important}
## Bias-Variance View

Fix a forecast point $x_0$ and suppose a new observation at that point is generated as
$$
...
+\underbrace{\operatorname{Bias}\left(\hat f(x_0)\right)^2}_{\text{approximation error}}
+\underbrace{\sigma^2}_{\text{irreducible}}.
$$
The independence assumption is what removes the cross-term between $\varepsilon_0$ and $\hat f(x_0)$; it holds because $Y_0$ is a genuinely new observation, which is precisely the assumption that in-sample evaluation violates.
...
where $|T|$ is the number of leaves in tree $T$ and $\alpha \geq 0$ is a tuning parameter. The minimization runs over a specific, finite domain: the **subtrees of the grown tree $T_0$** obtained by collapsing internal nodes (replacing a whole branch by a single leaf), not over all conceivable trees — which is what makes the search feasible at all. A classical result of @BreimanFriedmanOlshenStone1984 makes it efficient: as $\alpha$ increases from zero, the minimizing subtrees form a finite *nested* sequence $T_0 \supseteq T_1 \supseteq \cdots \supseteq \{\text{root}\}$, obtained by successively collapsing the "weakest link" — the internal node whose branch contributes least fit improvement per leaf. One therefore computes the entire sequence once and only has to choose $\alpha$, which should be tuned with the time-aware validation schemes discussed in the warning below.
...
A single split near the top of the tree changes the sample available to all later splits. As a result, small perturbations in the training data can generate a different sequence of splits and therefore a different fitted function. This high variance is the main reason single trees are rarely used as the final predictive model.
...
```{python}
#| label: fig-dt-fredqd
#| fig-cap: "Depth-3 regression tree fit to one-step-ahead GDP growth (annualized log-difference of GDPC1) using five FRED-QD predictors. Each internal node shows the split variable and threshold; leaf nodes show the mean predicted value in percentage points annualized (value) and the number of training observations (samples). Colors indicate the leaf prediction: darker shading corresponds to higher predicted growth. The tree is trained on the first 75% of available quarters."
#| echo: true
...
3. Consider the cost-complexity objective
   ...
   $$
   where $|T|$ is the number of leaves. For which values of $\alpha$ is the one-split tree from Part 2 preferred to the stump with no split (the no-split tree consisting of a single root leaf that predicts the full-sample mean)?
...
So the stump sum of squared errors (SSE) is
...
4. Suppose the cost of a false negative is $C_{FN}$ and the cost of a false positive is $C_{FP}$. Show that predicting default minimizes expected cost when
   ...
   $$
   Evaluate the threshold for $C_{FN}=4$ and $C_{FP}=1$. Then apply it to the leaves of *both* candidate splits from Parts 2 and 3, and report which leaves change their predicted class relative to the $0.5$ rule. Explain why a leaf changes in one split but not the other, and what this says about when the choice of threshold actually matters.

*Exam level: the required logarithms are supplied, so all four parts are calculator-free. Parts 1-3 are the impurity mechanics, and Part 4 adds the cost-sensitive threshold derivation together with its effect on the fitted classifier.*
:::
