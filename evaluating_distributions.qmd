...
In many economic and financial applications, predicting a single value (a "point forecast") is not enough. We often need to understand the full range of possible outcomes and their likelihoods. This requires us to move from point forecasts to **distribution forecasts**.
...
5. Finally, we connect these ideas back to **information theory**, especially cross-entropy and the Kullback–Leibler (KL) divergence.
...
#| fig-cap: "Visual interpretation of the LogS. A higher density at the realized outcome y results in a better score."
#| echo: true
...
Minimizing the average LogS is equivalent to minimizing the KL divergence between the model's predictive distribution $q$ and the true data-generating distribution $p$. This equivalence is the theoretical foundation for using the LogS in model selection and evaluation.
:::
...
&= \int (G(z)-F(z))^2\,dz
   + \underbrace{\int F(z)(1-F(z))\,dz}_{h_{\text{CRPS}}(F)}.
\end{aligned}
$$
where we used $\mathbb E[\mathbf{1}\{z \geq Y\}]=F(z)$ and $\operatorname{Var}(\mathbf{1}\{z \geq Y\})=F(z)(1-F(z))$.

- The term $h_{\text{CRPS}}(F)=\int F(z)(1-F(z))\,dz$ is the generalized "CRPS entropy" of $F$.
- The excess risk
$$D_{\text{CRPS}}(F,G)=\int_{-\infty}^{\infty} (F(z)-G(z))^2\,dz$$
is the **Cramér distance** between $F$ and $G$. It is nonnegative and vanishes only when the two CDFs agree everywhere, which is what makes CRPS strictly proper (minimized at $G(x)=F(x)$ for all $x$). Note the naming: the Cramér distance integrates the squared CDF gap against Lebesgue measure, $dz$, as displayed above. The Cramér–von Mises criterion integrates the same squared gap against $dF(z)$ and is a different functional.
...
- Diebold-Mariano tests for comparing forecast performance [@DieboldMariano1995] and the conditional predictive-ability extension of @GiacominiWhite2006, which accommodates time-varying or estimated forecasting models and is the standard reference in real-time forecast evaluation
