...
| **Measurement Error Sensitivity** | Can be high, especially where forecast density is small. | Less sensitive: score changes by at most the absolute outcome error, though rankings can change [@Kleen2024]. |
...
- Diebold-Mariano tests for comparing forecast performance [@DieboldMariano1995] and the conditional predictive-ability extension of @GiacominiWhite2006, which accommodates time-varying or estimated forecasting models and is the standard reference in real-time forecast evaluation; Exercise 4.3 below develops the serial-correlation issue these tests must address
...
is the **Cramér distance** between $F_0$ and $F$. It is nonnegative and vanishes only when the two CDFs agree everywhere: equality almost everywhere implies equality everywhere by right-continuity of CDFs. This makes CRPS strictly proper, minimized at $F(z)=F_0(z)$ for all $z$. Note the naming: the Cramér distance integrates the squared CDF gap against Lebesgue measure, $dz$, as displayed above. The Cramér–von Mises criterion integrates the same squared gap against $dF_0(z)$ and is a different functional.
...
:::{.callout-note title="If random forests are new to you"}

You can consult the [random forests chapter](random_forests.qmd#sec-random-forests), or simply think of a random forest here as another forecasting model that maps predictors to a prediction. Its internal workings are not needed to follow this illustration. The focus is on constructing forecasts of the conditional mean and variance, then evaluating the resulting predictive distributions using LogS, CRPS, and PIT histograms.
:::

We use the following notions in this section:
...
:::{.callout-note}
## Exercise 4.2: PIT Under Dispersion Misspecification
...
:::{.callout-note}
## Exercise 4.3: Comparing Average Scores Under Serial Correlation
