...
-   The LogS is sensitive to tail events and heavily penalizes a model that assigns low density at an outcome that occurred.
-   It requires an explicit forecast density $f(y)$, which is not always available.

**Visual Interpretation of the LogS**

The LogS depends on the **height of the density** at the realized outcome $y$. A forecast that places more density at the realized value receives a better (i.e. lower) score.
...
-   The CRPS generalizes the MAE to probabilistic forecasts. If the forecast is a single point, the CRPS reduces to the MAE. Even though it may not be intuitive at first sight given the square inside the integral, one can rewrite the CRPS in terms of two independent and identically distributed (i.i.d.) draws $X, X'$ from the forecast distribution [@GneitingRaftery2007],
$$ \text{CRPS}(F,y) = \mathbb{E} |X-y| - \frac{1}{2} \mathbb{E} |X-X'|, \quad X,X'\stackrel{iid}{\sim}F, $$
provided $F$ has a finite first moment (otherwise both expectations on the right are infinite).
...
4. Explain why serially correlated score differentials arise naturally in forecast evaluation — for example with multi-step or overlapping forecast targets — and why average-score comparisons such as those in @sec-eval-dist-empirical require both a HAC-type standard error and an appropriate weak-dependence central limit theorem before they can be interpreted inferentially [@DieboldMariano1995; @GiacominiWhite2006].
