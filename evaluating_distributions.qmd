...
-   **Distribution Forecasts**:
    ...
    -   Predict an entire probability distribution for the future outcome, e.g., $P(Y_{t+1} | \mathcal{F}_t)$, where $\mathcal{F}_t$ is the information set available at time $t$ (the forecast origin).
...
#| fig-cap: "Comparison of a point forecast and a distribution forecast for the same outcome. Left panel: the realized value (green vertical line) and a point forecast (red vertical line). Right panel: the same realized value against a full forecast density (blue curve with shading); the forecast now assigns probability to every possible outcome rather than naming a single one."
#| echo: true
import numpy as np
...
We call $S$ **strictly proper** if the true distribution is the *unique* minimizer. This is analogous to how the conditional mean is the unique forecast that minimizes the Mean Squared Error: proper scoring rules incentivize honest reporting of the entire predictive distribution.
...
-   The LogS is sensitive to tail events and heavily penalizes a model that assigns low probability to an outcome that occurred.
-   It requires an explicit forecast density $f(y)$, which is not always available.

**Visual Interpretation of the LogS**

The LogS depends on the **height of the density** at the realized outcome $y$. A forecast that assigns higher probability to the realized value receives a better (i.e. lower) score.
...
We identify each predictive distribution with its CDF, writing $Y\sim F$ and $F(z)=\Pr(Y\le z)$ interchangeably.
...
The CRPS measures the squared difference between the forecast CDF, $F(z)$, and the empirical CDF of the outcome, which is a step function at $y$. If the forecast places probability mass near the realization, the shaded area is smaller and the CRPS is better (i.e. lower).

```{python}
#| label: fig-crps-viz
#| fig-cap: "Visual interpretation of the CRPS. Each panel shows a forecast CDF (left panel: blue, concentrated near the outcome; right panel: red, concentrated away from it) against the step-function CDF of the realized outcome (green), with the region between the two curves shaded orange. The CRPS integrates the squared vertical gap between the curves, so the larger shaded region in the right panel corresponds to a higher (worse) score."
#| echo: true
...
**Intuition for the CRPS**:

-   The CRPS generalizes the MAE to probabilistic forecasts. If the forecast is a single point, the CRPS reduces to the MAE. Even though it may not be intuitive at first sight given the square inside the integral, one can rewrite the CRPS as [@GneitingRaftery2007],
$$ \text{CRPS}(F,y) = \mathbb{E} |X-y| - \frac{1}{2} \mathbb{E} |X-X'|, \quad X,X'\stackrel{iid}{\sim}F, $$
where $X$ and $X'$ are independent and identically distributed (i.i.d.) draws from the forecast distribution $F$, provided $F$ has a finite first moment (otherwise both expectations on the right are infinite).
...
Both are strictly proper scoring rules, each relative to a specific class of forecast distributions: the LogS relative to distributions with densities, and the CRPS relative to distributions with a finite first moment [@GneitingRaftery2007]. Within these classes, they have different sensitivities.
...
- Diebold-Mariano tests for comparing forecast performance [@DieboldMariano1995] and the conditional predictive-ability extension of @GiacominiWhite2006, which accommodates time-varying or estimated forecasting models and is the standard reference in real-time forecast evaluation; Exercise 3.4 below develops the serial-correlation issue these tests must address
...
Beyond comparing models with scoring rules, we often want to diagnose *how* a model's predictive distributions are failing. A powerful tool for this is the **PIT**.
...
-   **Miscalibrated Skewness**: If the histogram is asymmetric, it indicates a mismatch in skewness — though the signature is more subtle than a simple monotone slope. Suppose the outcomes are right-skewed while the forecast is symmetric with matched mean and variance. Realizations then cluster around the mode of the true distribution, which lies *below* the symmetric forecast's center, producing a hump to the left of $0.5$; the light left tail of the outcomes produces a deficit of PIT values near $0$; and the heavy right tail produces some extra mass near $1$. The model is systematically surprised by large positive outcomes but almost never by large negative ones, and the asymmetric histogram records both effects.
...
2.  **KL Divergence $D_{\text{KL}}(p \parallel q)$**: the extra loss incurred because $q$ differs from $p$. Since $D_{\text{KL}} \geq 0$, this term is the room for improvement (closely related to the [MLE-as-KL discussion](information_theory.qmd#sec-mle-kl-divergence)).
...
Readers who have not yet worked through the [random forests chapter](random_forests.qmd#sec-random-forests) only need the following idea for this application: a regression forest is a non-parametric ensemble method which combines multiple predictions into one. We use the following notions in this section:
...
We compare three density forecasts, all Gaussian. The **first-order autoregressive (AR(1)) baseline** sets the point forecast to a least-squares AR(1) in lagged GDP growth and the variance to the expanding in-sample residual variance — a quantity that stays roughly constant once enough data are available. The **heteroskedastic random forest (in-sample)** — we abbreviate random forest as RF below — uses two forests: a mean forest that predicts the conditional mean from all five predictors, and a variance forest that regresses the squared *in-sample* residuals of the first forest on the same predictors. This two-model construction is analogous to specifying a mean equation and a variance equation separately as, for example, done in generalized autoregressive conditional heteroskedasticity (GARCH) models. The **heteroskedastic random forest (OOB)** uses the same architecture but feeds the variance forest squared *out-of-bag* (OOB) residuals from the mean forest instead.[^hrf-note] We deliberately include both heteroskedastic variants because the two differ only in one design choice — how the residuals fed to the variance forest are produced — and that single choice will turn out to determine whether the model beats or loses to the AR(1) baseline.
...
@fig-eval-dist-summary condenses the evaluation: average scores in the first panel and PIT histograms for each model in the rest. Two reading aids matter with only $n=63$ test quarters. First, we use eight bins, so that roughly eight observations per bin are expected under uniformity; finer bins would mostly display noise. Second, the shaded band around the uniform density marks pointwise ±2-standard-error variation of the bin heights under correct calibration, so only bars escaping the band deserve attention. We inspect only the marginal shape of the PITs here; testing for leftover serial dependence in the PIT sequence — the second half of the diagnosis in @sec-eval-dist-pit — requires additional tools, and Exercise 3.4 develops the closely related serial-correlation problem for average-score comparisons.

```{python}
#| label: fig-eval-dist-summary
#| fig-cap: "Average LogS (solid bars) and CRPS (hatched bars) by model in the top-left panel, lower is better, and PIT histograms for each model over the $n=63$ test quarters in the remaining panels, using 8 bins. The dashed red line marks the uniform density; the shaded band is a pointwise ±2-standard-error region around it under correct calibration, based on the binomial variation of the bin counts. LogS and CRPS are on different scales, so bar heights in the top-left panel are comparable across models within a score, not between the two scores."
#| echo: true
...
:::{.callout-note}
## Exercise 3.1: Expected LogS Under Gaussian Misspecification
...
\sigma^2=\sigma_0^2.
$$
Hence the expected log score is minimized at the true mean and the true variance. (The stationary point is the global minimum: for any fixed $\mu$ the criterion diverges to $+\infty$ both as $\sigma^2\to 0$ — the second term blows up, since $\sigma_0^2>0$ — and as $\sigma^2\to\infty$, and for fixed $\sigma^2$ it increases without bound in $(\mu-\mu_0)^2$, so the unique stationary point is where the minimum is attained.)
...
:::{.callout-warning collapse="true"}
## Hint for Part 2
In the expression from Part 1, only one integral depends on the forecast $F$.
:::
...
:::{.callout-note}
## Exercise 3.4: Comparing Average Scores Under Serial Correlation

Two density forecasters, $A$ and $B$, issue one-step-ahead predictive distributions for the same target series, and both are evaluated with the same proper scoring rule. Let $d_t$ denote the score differential in period $t$ — the score of forecaster $A$ minus the score of forecaster $B$, each evaluated at the realized outcome — and assume $\{d_t\}$ is covariance stationary with mean $\delta=\mathbb{E}[d_t]$ and autocovariances $\gamma_\ell=\operatorname{Cov}(d_t,d_{t-\ell})$ satisfying $\sum_{\ell=0}^{\infty}|\gamma_\ell|<\infty$. Equal predictive ability is judged by comparing $\bar d=\frac{1}{T}\sum_{t=1}^{T}d_t$ with its standard error.
...
2. Suppose the differential has the moving-average structure
   $$
   d_t=\delta+\varepsilon_t+\theta\varepsilon_{t-1},
   ...
   $$
   T\operatorname{Var}(\bar d)\ \longrightarrow\ \sigma_\varepsilon^2(1+\theta)^2
   \qquad\text{as } T\to\infty.
   $$
3. An analyst ignores the dependence and uses the i.i.d.-based variance $\gamma_0/T$. Show that the ratio of the correct asymptotic variance to this naive variance is $(1+\theta)^2/(1+\theta^2)$, and evaluate it at $\theta=1$. Explain in two or three sentences why serially correlated score differentials arise naturally in forecast evaluation — for example with multi-step or overlapping forecast targets — and what your ratio implies for average-score comparisons, such as those in @sec-eval-dist-empirical, that are reported without a HAC-type standard error [@DieboldMariano1995; @GiacominiWhite2006].

*Exam level. The exercise transfers the HAC logic familiar from regression inference to proper-score comparisons: with positively autocorrelated loss differentials — the typical case for overlapping forecasts, as at $\theta=1$ — i.i.d. standard errors understate the uncertainty and overstate significance. Dependence can move the variance in either direction.*
:::
...
Write
$$
\operatorname{Var}(\bar d)=\frac{1}{T^2}\sum_{t=1}^{T}\sum_{s=1}^{T}\operatorname{Cov}(d_t,d_s)
$$
...
By stationarity, $\operatorname{Cov}(d_t,d_s)=\gamma_{|t-s|}$, so
$$
\operatorname{Var}(\bar d)
...
From $d_t-\delta=\varepsilon_t+\theta\varepsilon_{t-1}$,
$$
...
$$
which equals $4/2=2$ at $\theta=1$: the true variance of $\bar d$ is twice the naive one, so the naive standard error is too small by a factor $\sqrt{2}$ and the associated $t$-statistic is inflated by the same factor. (The direction is governed by the sign of the autocorrelation: at $\theta=-1/2$ the ratio is $0.25/1.25=0.2$, so the naive standard error would instead be too *large*. Positive autocorrelation is the empirically typical case for score differentials.)

Serially correlated differentials are the rule rather than the exception in forecast evaluation. An $h$-step-ahead forecast target overlaps its neighbors in $h-1$ innovations, so even a correctly specified model produces loss differentials with a moving-average structure of order $h-1$; estimated parameters and persistent volatility add further dependence. Average-score gaps like those in @sec-eval-dist-empirical therefore require a long-run (HAC-type) variance estimate before differences can be judged statistically significant — which is precisely the construction of the Diebold–Mariano test and its conditional extension by Giacomini and White.
:::
