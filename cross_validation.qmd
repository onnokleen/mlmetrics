...
**Model and flexibility.** Let $X_i$ denote the scalar predictor and $Y_i$ the outcome of unit $i$, and write $x_i$ and $y_i$ for their observed realizations, following the convention fixed in the [information theory chapter](information_theory.qmd). Consider a regression model whose errors are independent and identically distributed (i.i.d.) Gaussian conditional on the predictor:
...
The K-fold design introduced below addresses the first and third drawbacks: by averaging over $K$ splits, it reduces the dependence of the estimate on any one arbitrary split, and it lets every observation serve in validation exactly once. Cross-validation does not by itself prevent contamination from repeated model comparison. Selecting among many configurations using the same cross-validated loss estimates reuses those estimates just as repeated decisions reuse a test set, which is why the workflow below still separates selection from final assessment.

:::{.callout-important}
## Reliable Workflow: Validation and Test Sets {#sec-cv-reliable-workflow}
...
> In each CV split, every preprocessing or transformation rule used to build predictors or outcomes must be a function of *that split's training observations only*. Any rule whose computed value depends on the validation observations contaminates the validation distribution.
...
If these steps are computed on the full dataset before cross-validation, the validation folds share information with the training step. This leakage contaminates the validation experiment and can bias the reported performance estimate, often optimistically; the direction and magnitude depend on the transformation and optimization objective function. Exercise 2.1 below shows how a validation observation enters its own full-sample scaling rule and shrinks its standardized magnitude. It then distinguishes that change in the predictor from a change in prediction loss: refitted unpenalized OLS predictions are invariant to this affine rescaling, whereas a fixed penalty on the standardized slope need not preserve predictions.
:::
...
We fix the fitting procedure explicitly at no more than 1,000 training epochs per CV split, where one epoch is one full pass through the training fold. In this example, scikit-learn's `max_iter` counts epochs, while each weight update uses a mini-batch—a subset of the training fold. With 800 training observations per fold and the default mini-batch size of 200, a capped fit makes 4,000 gradient updates. The [optimization chapter](optimization.qmd#sec-optim-mini-batch) explains this distinction between epochs and updates.

Instead of suppressing convergence warnings, the code records how many of the five fits reach that cap for every configuration; a capped fit is a valid fixed-budget procedure, but its held-out loss can reflect incomplete optimization as well as architecture and learning rate. Within the manual grid-search loop, scikit-learn's `cross_validate` function evaluates one fixed pipeline configuration: it refits the pipeline on the training observations in each of the five splits and returns the corresponding held-out fold results. The surrounding `ParameterGrid` loop repeats this evaluation for every configuration, after which the code selects the configuration with the smallest mean cross-validated MSE. @fig-nn-cv-tuning summarizes the search: each point is one configuration in the grid, plotted against the learning rate in the left panel and against the architecture in the right panel, with the red star marking the selected configuration. Scikit-learn follows a higher-is-better `scoring` convention, so `neg_mean_squared_error` returns the negative of each fold's MSE; the minus sign in the code converts the average back to ordinary MSE before comparison and plotting.
...
print(f"Folds reaching 1,000 epochs: {int(best_config['n_capped_folds'])}/5")
...
The five configurations with the lowest mean CV MSE are reported separately in @tbl-nn-cv-top-five. Its first column uses exactly the same hidden-units-by-layer labels as the right panel of @fig-nn-cv-tuning.

```{python}
#| label: tbl-nn-cv-top-five
#| tbl-cap: "Five configurations with the lowest mean cross-validated MSE. Hidden units by layer uses the same architecture labels as @fig-nn-cv-tuning; capped folds counts the fold fits that reached the 1,000-epoch limit."
#| echo: true
...
The mean CV MSE ranks the candidate configurations. The **Across-fold MSE standard deviation** column—computed in the code as `fold_mse_sd` with `ddof=1`—describes how much the $K$ validation-fold MSE values fluctuate, but it is *not* a standard error for the cross-validated risk: the fold-average losses are correlated because their training sets overlap [@BatesHastieTibshirani2024]. The **Capped folds** column provides a separate optimization diagnostic. A positive value means that differences in CV MSE may reflect incomplete optimization under the fixed iteration budget, not only differences in architecture or learning rate. Small differences in mean CV MSE should therefore not be read as precise rankings, especially when across-fold dispersion is large or the number of capped fits differs.
:::

The grid search above uses a single level of cross-validation. For each fixed configuration, `cross_validate` produces five returned values that the code converts to validation-fold MSEs. The `ParameterGrid` loop repeats that call for every configuration, and the subsequent `idxmin` step selects the configuration with the smallest mean CV MSE. This entire selection step plays the role of the **inner tuning loop** in the nested procedure developed next. Using the same splits for every configuration makes their loss estimates directly comparable, but the ranking remains noisy, as the dispersion and convergence diagnostics emphasize. Moreover, the mean CV MSE of the *selected* configuration is optimistically low, for exactly the reason formalized in Exercise 2.2 below: taking the smallest of many noisy validation-loss estimates favors lucky draws. @sec-cv-nested develops the separate outer layer needed to assess the tuning procedure without reusing its inner loss estimates.

Exhaustive grid search becomes costly as the number of hyperparameters grows. Random and sequential search methods provide alternatives, which we study in the [chapter on advanced hyperparameter optimization](hpo.qmd). Regardless of how the candidates are searched, using the same validation-loss estimates for selection and assessment remains a separate problem.

## Nested Cross-Validation {#sec-cv-nested}

The practical example used cross-validation to answer a **selection question**: which candidate architecture and learning rate should we choose? Nested cross-validation adds an assessment layer for a second question: how well will the entire selection procedure predict on new data? Reporting the smallest inner-CV loss estimate as the answer reuses the same validation-loss estimates for both tasks. Because the minimum favors configurations that received unusually favorable validation draws, the reported estimate is generally optimistic even if each candidate's CV loss estimate is unbiased before selection [@VarmaSimon2006; @CawleyTalbot2010].

@fig-nested-cv visualizes how nested cross-validation prevents that reuse. The left panel shows the outer loop that produces the final assessment losses. The right panel zooms into the first outer split and shows the separate inner loop used for tuning.
...
The right panel of @fig-nested-cv makes the separation operational. The four inner folds rotate between training and validation, but $D_1^{\mathrm{out}}$ never changes role. After the inner loss estimates select a configuration, denoted $\hat\lambda_1$ below, the procedure refits on the complete outer training sample and evaluates once on $D_1^{\mathrm{out}}$. Repeating the same sequence for the other four outer splits produces one reliable held-out loss for every observation.
...
2. K-fold cross-validation estimates the expected risk of a fitting procedure trained on roughly $N(K-1)/K$ observations, not the realized conditional risk $R(\hat{f})$ of the one model refit on the full sample.
3. A validation strategy is reliable only if every data-dependent preprocessing step is recomputed using each split's training observations.
4. Repeatedly choosing the specification with the smallest noisy validation-loss estimate creates selection-induced optimism, even when each candidate's estimate is unbiased on its own.
...
- Reporting the smallest inner-CV loss estimate as the final performance estimate after searching over many configurations.
...
$$
Both variances use the divide-by-own-sample-size convention (`ddof=0`), as does `StandardScaler` in the chapter's Python example. Assume $N\ge3$ and $s_{-j}^2>0$.

1. Derive the mean and variance decompositions
   $$
   \bar{x}=\bar{x}_{-j}+\frac{x_j-\bar{x}_{-j}}{N},
   \qquad
   s^2=\frac{N-1}{N}s_{-j}^2+\frac{N-1}{N^2}(x_j-\bar{x}_{-j})^2.
   $$
2. Part 1 ensures $s>0$. Define $z_j^{\mathrm{full}}=(x_j-\bar{x})/s$ and $z_j^{(-j)}=(x_j-\bar{x}_{-j})/s_{-j}$. Show that
   $$
   z_j^{\mathrm{full}}=
   \frac{\sqrt{N-1}\,z_j^{(-j)}}{\sqrt{N+(z_j^{(-j)})^2}},
   $$
   and deduce $|z_j^{\mathrm{full}}|<|z_j^{(-j)}|$ whenever $z_j^{(-j)}\ne0$. Identify where the held-out observation enters the transformation.
3. Now suppose outcomes $y_1,\dots,y_N$ are observed. For any centering constant $m$ and scale $r>0$, fit the regression $\alpha+\gamma(x_i-m)/r$ by unpenalized OLS on $i\ne j$, including the intercept. Derive the one-to-one mapping between $(\alpha,\gamma)$ and the coefficients $(\beta_0,\beta_1)$ of $\beta_0+\beta_1x$. Use it to prove that choosing $(m,r)=(\bar{x},s)$ or $(\bar{x}_{-j},s_{-j})$ gives the same held-out prediction and squared prediction error after refitting.
4. Instead fit $(\alpha,\gamma)$ by minimizing
   $$
   \frac{1}{N-1}\sum_{i\ne j}\left[y_i-\alpha-\gamma\frac{x_i-m}{r}\right]^2+\lambda\gamma^2,
   \qquad \lambda>0,
   $$
   with the same fixed $\lambda$ under either scaling rule and no penalty on the intercept. Write $\bar y_{-j}=(N-1)^{-1}\sum_{i\ne j}y_i$ and $C_{xy,-j}=(N-1)^{-1}\sum_{i\ne j}(x_i-\bar x_{-j})(y_i-\bar y_{-j})$. Derive the fitted slope in the original $x$ units and the held-out prediction. Use Part 1 to determine when full-sample scaling increases the penalty weight in the original units. Explain why this does not determine whether squared validation loss rises or falls.

*Exam level. The exercise distinguishes a transformation that uses validation information from its effect on forecasts, comparing unpenalized and penalized least squares.*
:::

:::{.callout-warning collapse="true"}
## Hint for Part 1
For the variance calculation, write $x_i-\bar{x}=(x_i-\bar{x}_{-j})-(\bar{x}-\bar{x}_{-j})$ for $i\ne j$ and use $\sum_{i\ne j}(x_i-\bar{x}_{-j})=0$.
:::

:::{.callout-warning collapse="true"}
## Hint for Part 4
Express both the fitted values and the penalty in terms of $(\beta_0,\beta_1)$ from Part 3. Minimize first over the unpenalized intercept, then over the slope.
:::

:::{.callout-tip collapse="true"}
## Solution

**Part 1: Mean and Variance Decompositions**

Since $N\bar{x}=(N-1)\bar{x}_{-j}+x_j$,
$$
\bar{x}-\bar{x}_{-j}=\frac{x_j-\bar{x}_{-j}}{N},
\qquad
x_j-\bar{x}=\frac{N-1}{N}(x_j-\bar{x}_{-j}).
$$
For the remaining observations, centering at $\bar{x}_{-j}$ eliminates the cross term:
$$
\sum_{i\ne j}(x_i-\bar{x})^2
=(N-1)s_{-j}^2+(N-1)(\bar{x}-\bar{x}_{-j})^2.
$$
Adding $(x_j-\bar{x})^2$ and substituting the two mean identities gives
$$
Ns^2=(N-1)s_{-j}^2+
\left(\frac{N-1}{N^2}+\frac{(N-1)^2}{N^2}\right)(x_j-\bar{x}_{-j})^2.
$$
The coefficient in parentheses is $(N-1)/N$. Dividing by $N$ proves the stated variance decomposition.

**Part 2: Shrinkage of the Standardized Predictor**

Part 1 gives
$$
x_j-\bar{x}=\frac{N-1}{N}z_j^{(-j)}s_{-j},
\qquad
s=s_{-j}\sqrt{\frac{N-1}{N}}\sqrt{1+\frac{(z_j^{(-j)})^2}{N}}.
$$
...
\frac{\sqrt{N-1}\,z_j^{(-j)}}{\sqrt{N+(z_j^{(-j)})^2}}.
$$
For $z_j^{(-j)}\ne0$, the ratio of absolute standardized values is strictly below one because $N+(z_j^{(-j)})^2>N-1$. The held-out $x_j$ enters both the full-sample mean and variance. Its standardized magnitude therefore shrinks under a rule that uses validation information. Whether predictions change is a separate question.

**Part 3: Invariance of Refitted OLS Predictions**

For every $x$,
$$
\alpha+\gamma\frac{x-m}{r}
=\left(\alpha-\frac{\gamma m}{r}\right)+\frac{\gamma}{r}x.
$$
Thus
$$
\beta_0=\alpha-\frac{\gamma m}{r},\qquad \beta_1=\frac{\gamma}{r},
\qquad
\alpha=\beta_0+\beta_1m,\qquad \gamma=r\beta_1.
$$
Because $r>0$, this is a one-to-one reparameterization of the same set of affine prediction functions. The unpenalized training criterion is consequently the same function of $(\beta_0,\beta_1)$ under either scaling rule. The intercept and slope are uniquely determined because $s_{-j}^2>0$. Refitting therefore gives the same prediction at $x_j$ and the same squared error $(y_j-\hat f(x_j))^2$ under both transformations, despite Part 2's change in standardized magnitude.

**Part 4: A Fixed Penalty Makes Scale Matter**

The penalty becomes $\lambda r^2\beta_1^2$ in the original units. For any fixed slope, the intercept minimizing the criterion is
$$
\hat\beta_0=\bar y_{-j}-\beta_1\bar x_{-j}.
$$
Substituting it into the criterion leaves a term independent of $\beta_1$ plus
$$
-2\beta_1 C_{xy,-j}+\beta_1^2(s_{-j}^2+\lambda r^2).
$$
The coefficient of $\beta_1^2$ is strictly positive, so
$$
\hat\beta_1(r)=\frac{C_{xy,-j}}{s_{-j}^2+\lambda r^2},
\qquad
\hat f_{m,r}(x_j)=\bar y_{-j}+\hat\beta_1(r)(x_j-\bar x_{-j}).
$$
The centering constant $m$ cancels because the intercept is unpenalized. If the two scales differ, the predictions differ whenever $C_{xy,-j}(x_j-\bar x_{-j})\ne0$.

Part 1 gives
$$
s^2-s_{-j}^2=\frac{s_{-j}^2}{N^2}\left[(N-1)(z_j^{(-j)})^2-N\right].
$$
Full-sample scaling therefore increases the penalty weight $\lambda r^2$ exactly when $(z_j^{(-j)})^2>N/(N-1)$; below this threshold the weight decreases, and at equality it is unchanged. The direction of the penalty change is determined, but which prediction is closer to $y_j$ still depends on the outcome. Thus the change in squared validation loss can have either sign. Training-only preprocessing remains the general design rule, while Part 3 identifies a specific invariance of unpenalized OLS.
:::

:::{.callout-note}
## Exercise 2.2: Why Model Search Creates Optimistically Low Validation-Loss Estimates
...
\hat{R}_B = R_0+\eta_B,
$$
where expectations are taken over the validation sample with the fitted rules $\hat f_A$ and $\hat f_B$ held fixed, $\eta_A$ and $\eta_B$ have mean zero, and $\mathbb{P}(\eta_A\neq\eta_B)>0$, so the two estimates do not coincide with certainty. They need not be independent: candidate models evaluated on the same validation observations will generally have dependent loss-estimation errors.
...
3. Suppose in addition that $(\eta_A,\eta_B)$ is jointly Gaussian with
   ...
   $$
   Derive the exact expectation of the selected validation-loss estimate. Show that it equals
   $$
   \mathbb{E}\big[\min(\hat{R}_A,\hat{R}_B)\big]
   =
   R_0-\frac{\sigma\sqrt{1-c}}{\sqrt{\pi}},
   $$
   and explain why stronger positive dependence between candidate loss estimates reduces the selection-induced optimism. Specialize the result to independent errors.
...
5. An econometrician compares specifications $A$ and $B$ on the same validation sample, selects $\hat s\in\arg\min_{s\in\{A,B\}}\hat R_s$, and reports $\min(\hat R_A,\hat R_B)$ as the risk of the selected specification. Using Part 4 and $R_{\hat s}\geq\min(R_A,R_B)$, compare the expected reported loss estimate with $\mathbb E[R_{\hat s}]$. Then explain why evaluating the selected specification on an untouched test set, or in the outer loop of nested cross-validation, avoids this selection-induced optimism.

*Exam level. The exercise formalizes a central point of the chapter: validation is noisy, and searching across specifications makes the smallest observed loss estimate look too low.*
:::
...
\leq \min(R_A,R_B).
$$
Thus the exact-tie calculation in Parts 1–3 makes the mechanism transparent but is not required for the weak optimism inequality. More precisely, writing $x^+:=\max\{x,0\}$ for the positive part, the pointwise identity
$$
\min(\hat R_A,\hat R_B)
=\hat R_A-(\hat R_A-\hat R_B)^+,
$$
holds without any ordering assumption. If $R_A\leq R_B$, taking expectations gives
$$
\mathbb E[\min(\hat R_A,\hat R_B)]
=R_A-\mathbb E[(\hat R_A-\hat R_B)^+].
$$
...
Each candidate's validation-loss estimate is unbiased when viewed separately:
$$
\mathbb{E}[\hat{R}_s]=R_s,
\qquad s\in\{A,B\}.
$$
For the selected specification, Part 4 gives
$$
\mathbb E[\min(\hat R_A,\hat R_B)]
\leq \min(R_A,R_B)
\leq \mathbb E[R_{\hat s}].
$$
The reported minimum validation-loss estimate is therefore optimistic for the expected risk of the selected specification. An untouched test set avoids this bias because, conditional on the selected fitted rule, the test loss estimate is computed from fresh observations that played no role in forming $\hat s$. The outer validation fold in nested cross-validation serves the same role for the full selection procedure.
:::
