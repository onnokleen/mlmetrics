...
Let's see how to use cross-validation to select the best hyperparameters for a simple neural network, see [Section 5](feed_forward_nns.qmd) . We'll tune the number of hidden units and the learning rate using scikit-learn. Note that the sklearn implementation maximizes the ***negative mean squared error***; that is, higher cross-validation score means better performance.

```{python}
#| label: nn-cv-tuning
#| fig-cap: "Cross-validation results for neural network hyperparameter tuning. The whiskers in the plot "
#| echo: true
import numpy as np
import matplotlib.pyplot as plt
...
print(f"CV Score: {best_config['mean_cv_score']:.4f} ± {best_config['std_cv_score']:.4f}")
...
for alpha_val in results_df['alpha'].unique():
    ...
    ax1.scatter(subset['learning_rate'], subset['mean_cv_score'], 
...
for lr_val in results_df['learning_rate'].unique():
    ...
    ax2.scatter(subset['arch_pos'] + jitter[subset.index], subset['mean_cv_score'], 
...
print(results_df.nlargest(5, 'mean_cv_score')[['hidden_layers', 'learning_rate', 'alpha', 'mean_cv_score', 'std_cv_score']])
```
...
3. **Fair Comparison**: All models are evaluated on the same data splits, ensuring fair comparison
:::
