...
Typical choices made this way include the polynomial degree of this chapter's running example and the many structural and estimation settings of [neural networks](feed_forward_nns.qmd) and [tree-based methods](decision_trees.qmd) in later chapters.
...
The five configurations with the lowest mean CV MSE are reported separately in @tbl-nn-cv-top-five. Its first column uses exactly the same hidden-units-by-layer labels as the right panel of @fig-nn-cv-tuning. The **Fits at epoch limit** column counts how many of the five training runs reached 1,000 epochs; a value of 3 means that three runs used the full training budget.

```{python}
#| label: tbl-nn-cv-top-five
#| tbl-cap: "Five configurations with the lowest mean cross-validated MSE. Hidden units by layer uses the same architecture labels as @fig-nn-cv-tuning; Fits at epoch limit counts, out of five, the training runs that reached 1,000 epochs."
#| echo: true
...
4. Instead fit $(\alpha,\gamma)$ by minimizing
   ...
   \qquad \lambda>0,
   $$
   with the same fixed $\lambda$ under either scaling rule and no penalty on the intercept. Write $\bar y_{-j}=(N-1)^{-1}\sum_{i\ne j}y_i$ and $C_{xy,-j}=(N-1)^{-1}\sum_{i\ne j}(x_i-\bar x_{-j})(y_i-\bar y_{-j})$. Derive the fitted slope in the original $x$ units and the held-out prediction. Identify the coefficient multiplying $\beta_1^2$ in the penalty after reparameterization, and use Parts 1 and 2 to determine when full-sample scaling increases this coefficient. Holding the training observations and all predictors fixed, suppose the two scaling rules give different held-out predictions. Exhibit one value of $y_j$ for which full-sample scaling increases squared validation loss and another for which it decreases the loss.
...
:::{.callout-warning collapse="true"}
## Hint for Part 4
Express both the fitted values and the penalty in terms of $(\beta_0,\beta_1)$ from Part 3. Minimize first over the unpenalized intercept, then over the slope. For the loss comparison, consider choosing $y_j$ equal to one of the fitted predictions.
:::
...
Since $\gamma=r\beta_1$ by Part 3, the penalty becomes $\lambda r^2\beta_1^2$, with coefficient $\lambda r^2$ multiplying $\beta_1^2$. For any fixed slope, the intercept minimizing the criterion is
$$
...
$$
The centering constant $m$ cancels because the intercept is unpenalized. The two held-out predictions differ if and only if $s\ne s_{-j}$ and $C_{xy,-j}(x_j-\bar x_{-j})\ne0$.

Part 1 gives
$$
s^2-s_{-j}^2=\frac{(N-1)(x_j-\bar x_{-j})^2-Ns_{-j}^2}{N^2}.
$$
Since $\lambda>0$, full-sample scaling increases the coefficient $\lambda r^2$ exactly when
$$
(x_j-\bar x_{-j})^2>\frac{N}{N-1}s_{-j}^2
\quad\Longleftrightarrow\quad
(z_j^{(-j)})^2>\frac{N}{N-1},
$$
where the second form uses Part 2. Below this threshold the coefficient decreases, and at equality it is unchanged.

For the loss comparison, hold the training observations and all predictors fixed and suppose the two predictions differ. Neither fitted prediction uses $y_j$, so choosing its value does not change either fit:

- If $y_j=\hat f_{\bar x_{-j},s_{-j}}(x_j)$, the training-only rule has zero squared error and the full-sample rule has strictly positive squared error, so full-sample scaling increases validation loss.
- If $y_j=\hat f_{\bar x,s}(x_j)$, the full-sample rule has zero squared error and the training-only rule has strictly positive squared error, so full-sample scaling decreases validation loss.

If the predictions coincide, both rules give the same loss for every $y_j$. Training-only preprocessing remains the general design rule, while Part 3 identifies a specific invariance of unpenalized OLS.
:::
