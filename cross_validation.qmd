...
1. Derive the mean and variance decompositions
   ...
   $$
2. Define $z_j^{\mathrm{full}}=(x_j-\bar{x})/s$ and $z_j^{(-j)}=(x_j-\bar{x}_{-j})/s_{-j}$. Show that
   $$
   z_j^{\mathrm{full}}=
...
4. Instead fit $(\alpha,\gamma)$ by minimizing
   ...
   \qquad \lambda>0,
   $$
   with the same fixed $\lambda$ under either scaling rule and no penalty on the intercept. Write $\bar y_{-j}=(N-1)^{-1}\sum_{i\ne j}y_i$ and $C_{xy,-j}=(N-1)^{-1}\sum_{i\ne j}(x_i-\bar x_{-j})(y_i-\bar y_{-j})$. Derive the fitted slope in the original $x$ units and the held-out prediction. Explain why changing $r$ can now change validation loss, and why Part 2 alone cannot determine the direction of that change.
...
$$
The centering constant $m$ cancels because the intercept is unpenalized. The scale $r$ changes the effective penalty in the original units. If the two scales differ, the predictions can differ whenever $C_{xy,-j}(x_j-\bar x_{-j})\ne0$. Which prediction is closer to $y_j$ depends on the outcome; shrinking the standardized predictor alone does not determine whether squared validation loss rises or falls. Training-only preprocessing remains the general design rule, while Part 3 identifies a specific invariance of unpenalized OLS.
:::
