...
The K-fold design introduced below addresses the first and third drawbacks: by averaging over $K$ splits, it reduces the dependence of the estimate on any one arbitrary split, and it lets every observation serve in validation exactly once. Cross-validation does not by itself prevent contamination from repeated model comparison. Selecting among many configurations using the same cross-validated scores reuses those scores just as repeated decisions reuse a test set, which is why the workflow below still separates selection from final assessment.
...
**What does $\widehat{\mathrm{CV}}_K$ estimate?** With equal folds, each fold model is trained on $N(K-1)/K$ observations. Under the independent sampling assumed here, $\widehat{\mathrm{CV}}_K$ estimates the average out-of-sample risk of the fitting procedure at that training-sample size, where the average is over hypothetical training samples and fresh test observations. It does not estimate the realized risk $R(\hat f)$ of the particular model refitted on the observed full sample. @BatesHastieTibshirani2024 show how stark this distinction can be: in their homoskedastic Gaussian linear-model setting under squared loss, the CV estimate is conditionally independent of the realized conditional prediction error for linearly invariant estimators; under proportional Gaussian-design asymptotics, the corresponding correlation tends to zero. These special results are not universal properties of cross-validation, but they rule out interpreting CV as merely a noisy estimate of the risk of the model at hand.
...
The architecture labels below report how many computational units (often called neurons) each hidden layer contains. Thus, `50` denotes one hidden layer with 50 units, while `100-50` denotes two hidden layers with 100 and 50 units, in that order. These labels identify candidate architectures; width and depth both affect model capacity, so the labels do not place the candidates on a single ordered flexibility scale.
...
print(f"Fold-score standard deviation (ddof=1): {best_config['fold_mse_sd']:.4f}")
...
The five best-scoring configurations are reported separately in @tbl-nn-cv-top-five. Its first column uses exactly the same hidden-units-by-layer labels as the right panel of @fig-nn-cv-tuning.
...
top_five = results_df.nsmallest(5, 'mean_cv_mse')[[
    ...
    'fold_mse_sd': 'Fold-score standard deviation',
...
display(
    ...
    .format({
        'Learning rate': '{:.3g}',
        'Mean CV MSE': '{:.2f}',
        'Fold-score standard deviation': '{:.2f}',
        'Capped folds': '{:.0f}',
    })
)
```
...
The right panel of @fig-nested-cv makes the separation operational. The four inner folds rotate between training and validation, but $D_1^{\mathrm{out}}$ never changes role. After the inner scores select a configuration, denoted $\hat\lambda_1$ below, the procedure refits on the complete outer training sample and evaluates once on $D_1^{\mathrm{out}}$. Repeating the same sequence for the other four outer splits produces one reliable outer score for every observation.
...
5. An econometrician compares specifications $A$ and $B$ on the same validation sample, selects $\hat s\in\arg\min_{s\in\{A,B\}}\hat R_s$, and reports $\min(\hat R_A,\hat R_B)$ as the risk of the selected specification. Using Part 4 and $R_{\hat s}\geq\min(R_A,R_B)$, compare the expected reported score with $\mathbb E[R_{\hat s}]$. Then explain why evaluating the selected specification on an untouched test set, or in the outer loop of nested cross-validation, avoids this selection-induced optimism.
