...
2. We introduce a **honest workflow**—separating training, validation, and test sets—and explain when each set should be touched.
...
**What does $\widehat{\mathrm{CV}}_K$ estimate?** With equal folds, each fold model is trained on $N(K-1)/K$ observations. Under the independent sampling assumed here, $\widehat{\mathrm{CV}}_K$ estimates the average out-of-sample risk of the fitting procedure at that training-sample size, where the average is over hypothetical training samples and fresh test observations. It does not estimate the realized risk $R(\hat f)$ of the particular model refitted on the observed full sample. In the homoskedastic Gaussian linear model under squared loss, @BatesHastieTibshirani2024 prove an even sharper separation for linearly invariant estimators: conditional on the design matrix, the cross-validation error estimate is independent of the realized conditional prediction error. Under proportional Gaussian-design asymptotics, the correlation between those two quantities tends to zero. These are special-model results, not a universal property of cross-validation, but they reinforce the interpretation of cross-validation as an estimate of procedure risk rather than an approximate estimate of the error of one particular fit.
...
#| tbl-cap: "Five configurations with the lowest mean cross-validated MSE. Hidden units by layer uses the same architecture labels as @fig-nn-cv-tuning; capped folds counts the fold fits that reached the 1,000-iteration limit."
#| echo: true
...
$$
where $\bar{x}$ is the full-sample mean and $\bar{x}_{-j}$ is the mean computed only from the remaining $N-1$ training observations.
...
Both quantities use the divide-by-own-sample-size convention (`ddof=0`): the full sample contains $N$ observations, whereas the training subset contains $N-1$. This is also the convention used by `StandardScaler` in the chapter's Python example.

Assume $N\ge 3$ and that the remaining observations $\{x_i\}_{i\neq j}$ are not all equal, so that $s_{-j}^2>0$.
