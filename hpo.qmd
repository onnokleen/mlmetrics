...
The inequality is strict unless the same configuration always wins. After search, the winning validation score is partly low because the model is good and partly low because its realized validation noise was favorable.
:::
...
where $C(\lambda)$ denotes the surrogate's random prediction of the loss at $\lambda$, i.e. the random variable with $C(\lambda)\mid \mathcal{D}_t \sim N(m_t(\lambda), s_t(\lambda)^2)$, and $c_{\min,t} = \min_{1 \le j \le t} \hat c_j$, the smallest validation loss observed so far in $\mathcal{D}_t$.
...
where $y$ denotes the observed validation loss and $\gamma$ is the same threshold quantile used in the TPE algorithm box below.
...
*Exam-level. Part 2 is the computational core and supplies the normal-table values it needs; Parts 1, 3, and 4 are shorter and work best as scaffolding around it.*
:::
...
*Exam-level. Parts 1 and 2 are calculator-free arithmetic on the successive-halving schedule; Parts 3 and 4 test whether students see when cheap early evaluations mislead.*
:::
