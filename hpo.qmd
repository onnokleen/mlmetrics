...
1. Let $R$ denote the true out-of-sample loss of each configuration (assume all 12 share the same true risk for simplicity) and let $\hat{R}_k = R + \varepsilon_k$, $k=1,\dots,12$, where the $\varepsilon_k$ are iid mean-zero noise terms. Using the identity $\min(a,b)=\frac{a+b-|a-b|}{2}$, or otherwise, show that $\mathbb{E}[\min_k \hat{R}_k] < R$. Explain what this implies about the reported validation loss of the selected configuration.
...
2. In a Gaussian-process BO scheme, the current best observed validation loss is $f^\star=0.45$. The surrogate model predicts:
   - Configuration A: posterior mean $\mu_A=0.42$, posterior standard deviation $\sigma_A=0.01$.
   - Configuration B: posterior mean $\mu_B=0.47$, posterior standard deviation $\sigma_B=0.10$.

   Expected improvement is $\mathrm{EI}(\lambda)=\sigma\left[z\,\Phi(z)+\phi(z)\right]$ where $z=(f^\star-\mu)/\sigma$, and $\Phi$, $\phi$ are the standard normal CDF and PDF. Using $\Phi(3)\approx 0.999$, $\phi(3)\approx 0.004$, $\Phi(-0.2)\approx 0.421$, and $\phi(-0.2)\approx 0.391$, compute $\mathrm{EI}$ for both configurations and explain which one the acquisition function prefers.
...
Compute $z_A=(0.45-0.42)/0.01$ and $z_B=(0.45-0.47)/0.10$, then plug into the EI formula.
:::
...
$$
Configuration B has slightly higher EI despite a worse posterior mean. The high posterior uncertainty creates enough probability of beating $f^\star$ that the exploration payoff outweighs A's lower point prediction. This is why expected improvement naturally balances exploitation and exploration.
...
Full-budget random search would spend the full 81-epoch, 9-hour budget on every sampled configuration, including obviously poor ones. Hyperband spends only a small initial budget on many candidates and discards weak configurations early. It therefore concentrates long training runs on a much smaller set of promising candidates.
