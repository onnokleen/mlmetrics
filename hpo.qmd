...
Suppose validation runs are very expensive, the search space has four mainly continuous hyperparameters, and you can afford only 25 trials. Why might Bayesian optimization be preferable to plain random search?
:::

:::{.callout-tip collapse="true" title="Suggested Answer"}

With only 25 trials, each evaluation is valuable. Bayesian optimization uses previous outcomes to guide later proposals toward regions that look promising or uncertain, so it can often extract more information from a small budget than random search. This advantage is strongest when the search space is not too high-dimensional and the objective is not completely erratic.
:::
...
1. Explain why the minimum average validation loss across the 12 configurations is typically an optimistically biased estimate of the true out-of-sample loss of the selected configuration, even if each configuration's validation loss is a reasonable estimate when viewed in isolation.
...
The selected model is the one with the most favorable realized validation noise among the 12 candidates.
:::
...
Even if the expanding-window validation loss for each fixed configuration is a reasonable estimate of that configuration's forecasting risk, the minimum across 12 configurations is not an unbiased estimate of the selected model's true out-of-sample loss. The reason is selection bias: the chosen configuration is selected partly because it is genuinely good and partly because its realized validation noise happened to be favorable. Once we optimize over many candidates, the winner tends to look too good on the same validation design that was used to select it. This is the HPO analogue of specification search.
...
2. In a Gaussian-process BO scheme, consider two candidate configurations. Configuration A has a lower predicted validation loss than Configuration B, but Configuration B has much higher posterior uncertainty. Why might an expected-improvement criterion still choose Configuration B?
...
Expected improvement values both a low predicted loss and the possibility of beating the current best because of uncertainty.
:::
...
**Part 2: Why expected improvement can prefer the more uncertain point**

Expected improvement is not based only on the posterior mean. A configuration with higher uncertainty can still have a larger probability of producing a loss better than the current best observed value. Configuration B may therefore be preferred if its upside from learning is large enough, even though its mean prediction is slightly worse.
...
A neural network is trained to predict firm default probabilities from panel data. Training one configuration to completion takes 9 hours. You consider Hyperband with elimination rate $\eta = 3$.
...
2. Explain why Hyperband can be much more computationally efficient than full-budget random search in this setting.
...
Ask what happens to clearly poor configurations under ordinary random search.
:::
...
**Part 2: Why Hyperband can be efficient**

Full-budget random search would spend 9 hours on every sampled configuration, including obviously poor ones. Hyperband spends only a small initial budget on many candidates and discards weak configurations early. It therefore concentrates long training runs on a much smaller set of promising candidates.
