...
@fig-hpo-nested-time-series gives a time-series version of this logic. The outer split determines where honest evaluation occurs. Inside the outer training window, the researcher runs a second, time-aware tuning exercise. Only after the inner loop selects hyperparameters is the model refit on the full outer training sample and evaluated once on the untouched outer validation block.
...
where $C(\lambda)$ denotes the surrogate's random prediction of the loss at $\lambda$, i.e. the random variable with $C(\lambda)\mid \mathcal{D}_t \sim \mathcal{N}(m_t(\lambda), s_t(\lambda)^2)$, and the incumbent $c_{\min,t}$ is the best value seen so far.

Defining that incumbent requires a choice, and it is exactly the one this chapter opened with. The conventional definition $c_{\min,t} = \min_{1 \le j \le t} \hat c_j$ uses the smallest *observed* validation loss — which, by the selection-bias argument above, is optimistically biased whenever the scores are noisy. Expected improvement is then measured against a target that is too low, which makes the acquisition conservative: improvements look harder to achieve than they are, and the search under-explores. The standard remedy under noisy observations is the plug-in incumbent
$$
...
**Tree-structured Parzen estimator (TPE).** TPE is often used as a more practical Bayesian-optimization (BO) alternative. Instead of modeling the loss as a function of $\lambda$ directly, TPE splits past trials into a good set and a bad set using a loss threshold $y^\ast$. Let $y$ denote the observed validation loss of a trial. The threshold $y^\ast$ is chosen as the $\gamma$-quantile of the observed losses, so that a fraction $\gamma$ of past trials fall into the good set. TPE then fits two densities:
...
2. Split the evaluated archive into a good set and a bad set using a quantile threshold $\gamma$ on the observed validation loss, i.e. set the threshold $y^\ast$ equal to the $\gamma$-quantile of the observed losses.
...
1. For rounds $t = 0,1,\ldots,J$, fit the current $p^{[t]}$ configurations using budget $r^{[t]}$, where $J$ is the terminal round index at which the procedure stops.
...
Here $B$ is the resource budget allocated to *each* bracket, measured in the same units as $R$ — so a bracket spends roughly what it would cost to train $s_{\max}+1$ configurations to the full budget. Since Hyperband runs $s_{\max}+1$ brackets, one full sweep costs approximately $(s_{\max}+1)B$ in total.

Hyperband then runs several successive-halving brackets indexed by $s=s_{\max},\ldots,0$. The bracket index $s$ is distinct from the terminal round index $J$ in the successive-halving box above. A common parameterization is
...
@fig-hyperband-brackets shows how these brackets differ for $R=27$ and $\eta=3$, so that $s_{\max}=3$ and $B=108$. The wide bracket starts with many configurations and a very small budget, while the deep bracket starts with fewer configurations but allocates more resources immediately. The two features worth checking against the formula are that the starting counts $n_s$ fall as $s$ decreases (27, 12, 6, 4) while the initial resource levels $r_s$ rise (1, 3, 9, 27), and that the resulting total spend is roughly equal across brackets. That near-equality is deliberate: it is what allows Hyperband to hedge across breadth-versus-depth tradeoffs without committing more compute than a single successive-halving run of comparable size.

```{python}
#| label: fig-hyperband-brackets
#| fig-cap: "Three of the four Hyperband brackets for $R=27$ and $\\eta=3$, which give $s_{\\max}=3$ and $B=108$. Each rounded box reports the number of surviving configurations at a stage and the budget allocated to each one. Starting counts follow the formula above: $n_3=27$, $n_2=12$, $n_1=6$ (the omitted bracket $s=0$ evaluates $n_0=4$ configurations at the full budget 27 with no elimination). Moving from left to right within a bracket, configurations are eliminated at rate $\\eta$ and the budget per survivor is multiplied by $\\eta$. Bracket $s$ runs $s+1$ stages, so the wide bracket starts broad and cheap while the deep bracket starts narrow and expensive. Total spend per bracket is roughly constant by construction: 108, 99, and 108 configuration-epochs respectively."
#| echo: true
...
2. **Choose a validation design that respects the information set.** For independent and identically distributed (i.i.d.) data, $K$-fold cross-validation may be adequate. For dependent data, use expanding windows, rolling windows, or purged time-aware splits as appropriate.
...
1. Let $R$ denote the common true out-of-sample loss of all 12 configurations, and let $\hat{R}_k = R + \varepsilon_k$, $k=1,\dots,12$, where the $\varepsilon_k$ are independent and identically distributed (i.i.d.) mean-zero noise terms that are non-degenerate, so that $\Pr(\varepsilon_1 \neq \varepsilon_2) > 0$. First show, using the identity $\min(a,b)=\frac{a+b-|a-b|}{2}$, that for two configurations we have $\mathbb{E}[\min(\hat{R}_1,\hat{R}_2)] < R$. Then explain why this implies $\mathbb{E}[\min_k \hat{R}_k] < R$ for 12 configurations, and interpret the result for the reported validation loss of the selected configuration.
...
*Exam-level. Part 1 is a short derivation that carries the whole selection-bias point; Parts 2 and 3 compare validation designs; Part 4 is the econometric-diagnostic part on overlapping forecast targets.*
:::
...
$$
since non-degeneracy gives $|\varepsilon_1-\varepsilon_2|>0$ with positive probability, so its expectation is strictly positive. (Without that assumption the noise could be identically zero and the inequality would hold only weakly.) The same logic extends by induction: for any $K\ge 2$, $\mathbb{E}[\min_k \hat{R}_k]\le \mathbb{E}[\min(\hat{R}_1,\hat{R}_2)]<R$, because adding candidates can only lower the minimum further.
...
1. **Derive the closed form of expected improvement.** Under the Gaussian surrogate $C(\lambda)\mid\mathcal{D}_t \sim \mathcal{N}(m,s^2)$ with $s>0$, start from the definition
   ...
   $$
   Then read off which term rewards a low predicted loss and which rewards uncertainty, and state what happens to $\operatorname{EI}$ as $s\downarrow 0$ with $m>c_{\min}$ held fixed.
...
3. **Show why TPE ranks candidates by the density ratio.** TPE models the trials as $p(\lambda \mid y) = \ell(\lambda)$ if $y \le y^\ast$ and $g(\lambda)$ if $y > y^\ast$, with $\Pr(y \le y^\ast) = \gamma$. Using $p(\lambda) = \gamma\,\ell(\lambda) + (1-\gamma)\,g(\lambda)$ and the definition
   $$
   ...
   =
   \frac{K}{\gamma + (1-\gamma)\,g(\lambda)/\ell(\lambda)}
   $$
   for a constant $K$ that does not depend on $\lambda$. Conclude that maximizing $\operatorname{EI}$ is equivalent to maximizing $\ell(\lambda)/g(\lambda)$. Then apply the result: for candidates with ratios $\ell/g$ equal to $1.2$, $2.8$, and $0.9$, which does TPE evaluate next?
...
*Exam-level. Parts 1 and 3 are the derivations — the Gaussian closed form and the TPE density-ratio criterion — Part 2 is the numerical comparison and supplies the normal-table values it needs, and Part 4 closes with the design conditions.*
:::
...
The first term, $(c_{\min}-m)\Phi(z)$, is the exploitation term: it is large when the predicted loss $m$ sits well below the incumbent, weighted by the probability of improving. The second term, $s\phi(z)$, is the exploration term: it is proportional to the posterior standard deviation and rewards uncertainty regardless of the sign of $c_{\min}-m$.

As $s \downarrow 0$ with $m > c_{\min}$ fixed, we have $z \to -\infty$, and both $\Phi(z)$ and $\phi(z)$ go to zero — the second faster than any polynomial — so $\operatorname{EI} \to 0$. A point the surrogate is *certain* is worse than the incumbent has no expected improvement at all, which is exactly the behavior an acquisition rule should have.
...
**Part 3: The TPE density-ratio criterion**
...
=
\frac{\ell(\lambda)}{p(\lambda)}\underbrace{\int_{-\infty}^{y^\ast}(y^\ast-y)\,p(y)\,dy}_{=:\,K},
$$
where $K$ depends only on $y^\ast$ and the marginal distribution of the losses — not on $\lambda$. Substituting the mixture $p(\lambda) = \gamma\ell(\lambda) + (1-\gamma)g(\lambda)$ and dividing numerator and denominator by $\ell(\lambda)$,
$$
\operatorname{EI}(\lambda)
=
\frac{K\,\ell(\lambda)}{\gamma\ell(\lambda)+(1-\gamma)g(\lambda)}
=
\frac{K}{\gamma + (1-\gamma)\,g(\lambda)/\ell(\lambda)} .
$$
Because $K>0$ and $\gamma \in (0,1)$, the right-hand side is a strictly increasing function of $\ell(\lambda)/g(\lambda)$: the ratio enters only through $g/\ell$ in the denominator. Maximizing expected improvement is therefore *equivalent* to maximizing the density ratio, which is why TPE never needs to model the loss surface itself — it only has to rank candidates by how much more likely they are under the good-trial density than the bad-trial density.

Applying this to the three candidates: the ratios are $1.2$, $2.8$, and $0.9$, so TPE evaluates the second one next. It looks relatively likely under the density fitted to historically good trials and relatively unlikely under the bad-trial density, so the algorithm concentrates scarce validation budget where past evidence is most promising.
...
3. **Generalize the cost calculation.** Consider a successive-halving bracket that starts with $p^{[0]}$ configurations at budget $r^{[0]}$ and runs $J+1$ rounds, with the exact recursions $p^{[t]} = p^{[0]}\eta^{-t}$ and $r^{[t]} = r^{[0]}\eta^{t}$ (ignore rounding). Show that the cost of every round is the same, and hence that the bracket's total cost is $(J+1)\,p^{[0]}r^{[0]}$. Explain how this result accounts for the near-equal total spend across the Hyperband brackets shown in @fig-hyperband-brackets, and what it implies about the breadth-versus-depth tradeoff: what does a bracket give up when it starts with more configurations at the same total cost?
4. Suppose validation loss is very noisy in the first few epochs and some good models learn slowly. What specific risk does this create for Hyperband?
5. Compare Hyperband, random search, and Bayesian optimization for this problem. Under what condition would Hyperband clearly have the advantage?

*Exam-level. Parts 1 and 2 are calculator-free arithmetic on the successive-halving schedule, Part 3 turns that arithmetic into the general constant-cost-per-round result that motivates Hyperband's bracket design, and Parts 4 and 5 test whether students see when cheap early evaluations mislead.*
:::
...
**Part 3: Constant cost per round**

The cost of round $t$ is the number of surviving configurations times the budget each receives:
$$
p^{[t]} r^{[t]}
=
...
p^{[0]}r^{[0]},
$$
because the factors $\eta^{-t}$ and $\eta^{t}$ cancel exactly. The elimination rate and the budget-inflation rate are the same $\eta$, so every round spends the same amount: what is saved by discarding configurations is spent again on the survivors. Summing over the $J+1$ rounds,
$$
\sum_{t=0}^{J} p^{[t]}r^{[t]} = (J+1)\,p^{[0]}r^{[0]} .
$$
The Part 2 numbers are the special case $p^{[0]}=27$, $r^{[0]}=3$, $J=3$: four rounds of $81$ each, totaling $324$.

This is exactly why the Hyperband brackets in @fig-hyperband-brackets cost about the same. Bracket $s$ runs $s+1$ rounds starting from $n_s$ configurations at resource $r_s = R\eta^{-s}$, and the parameterization $n_s = \lceil (B/R)\eta^s/(s+1)\rceil$ is chosen precisely so that $(s+1)n_sr_s \approx B$ for every $s$ — the $\eta^s$ in $n_s$ cancels the $\eta^{-s}$ in $r_s$, and the division by $s+1$ offsets the number of rounds.
...
**Part 4: Risk from noisy early curves**
...
**Part 5: Comparison across methods**
