...
1. For rounds $t = 0,1,\ldots,s$, fit the current $p^{[t]}$ configurations using budget $r^{[t]}$, where $s$ is the terminal round index at which the procedure stops.
...
Hyperband then runs several successive-halving brackets indexed by $s=s_{\max},\ldots,0$. Note that $s$ is reused here as the bracket index, a different role from the terminal round index $s$ in the successive-halving box above. A common parameterization is
...
3. **Generalize the cost calculation.** Consider a successive-halving bracket that starts with $p^{[0]}$ configurations at budget $r^{[0]}$ and runs $s+1$ rounds, with the exact recursions $p^{[t]} = p^{[0]}\eta^{-t}$ and $r^{[t]} = r^{[0]}\eta^{t}$ (ignore rounding). Show that the cost of every round is the same, and hence that the bracket's total cost is $(s+1)\,p^{[0]}r^{[0]}$. Explain how this result accounts for the near-equal total spend across the Hyperband brackets shown in @fig-hyperband-brackets, and what it implies about the breadth-versus-depth tradeoff: what does a bracket give up when it starts with more configurations at the same total cost?
...
p^{[0]}r^{[0]},
$$
because the factors $\eta^{-t}$ and $\eta^{t}$ cancel exactly. The elimination rate and the budget-inflation rate are the same $\eta$, so every round spends the same amount: what is saved by discarding configurations is spent again on the survivors. Summing over the $s+1$ rounds,
$$
\sum_{t=0}^{s} p^{[t]}r^{[t]} = (s+1)\,p^{[0]}r^{[0]} .
$$
The Part 2 numbers are the special case $p^{[0]}=27$, $r^{[0]}=3$, $s=3$: four rounds of $81$ each, totaling $324$.
