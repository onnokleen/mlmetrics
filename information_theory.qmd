...
#| fig-cap: "Entropy of a Bernoulli random variable as a function of the success probability $\\theta$, using natural logarithms. The dashed lines mark the maximizer $\\theta=0.5$ and the maximum value $\\log 2 \\approx 0.69$. Entropy falls to zero at both endpoints, where the outcome is deterministic."
#| echo: true
...
By convention the cross-entropy is $+\infty$ whenever $q(x)=0$ but $p(x)>0$.
...
Here $\text{NLL}(q,x) := -\log q(x)$ is the negative log-likelihood contribution of a single observation $x$ evaluated under $q$, and $\mathbb{E}_{X\sim p}[f(X)]=\sum_{x\in\mathcal{X}} f(x)p(x)$ (resp. $\int f(x)p(x)\,dx$) denotes expectation when $X$ has distribution $p$.
...
The two names of the section title — relative entropy and KL divergence — are used interchangeably. Note: the KL divergence is not a metric, since it is asymmetric and violates the triangle inequality. As with cross-entropy, the divergence is finite only if $q(x)=0$ implies $p(x)=0$ (i.e. $p$ is absolutely continuous w.r.t. $q$); otherwise $D_{\text{KL}}(p\parallel q)=+\infty$.
...
- $h(p)$ is the entropy of $p$, measuring its irreducible uncertainty
- $h_{ce}(p,q)$ is the cross-entropy, measuring the expected log-loss of $q$ when $p$ is the true distribution
...
The KL representation provides an information-theoretic justification for the MLE principle. In words: maximum likelihood chooses the model whose implied distribution is closest, in KL divergence, to the empirical distribution of the data. Under misspecification, and given the regularity conditions needed for consistency, the probability limit of the MLE is the parameter value that minimizes KL divergence to the true distribution — this is the quasi-MLE interpretation of @White1982.
:::
...
#| fig-cap: "Six bivariate data-generating processes, $n=300$ observations each, with the sample Pearson correlation $r$ and an estimate of the mutual information reported above each panel. Every panel except the circular one adds Gaussian noise with the same standard deviation $0.3$, so the MI values are roughly comparable across panels; the circular panel instead perturbs the radius. Mutual information is estimated by a $k$-nearest-neighbour method (`sklearn.feature_selection.mutual_info_regression`), which truncates negative estimates at zero. All five dependent panels except the first are constructed to have zero population correlation."
#| echo: true
...
Two cautions about reading the numbers. First, they are *estimates*, not population quantities. Mutual information for continuous variables has to be estimated nonparametrically, which is considerably harder than estimating a correlation, and the estimator carries a small upward bias: the independent panel returns $0.053$ rather than $0.000$. That bias is why mutual information is a poor instrument for formally *testing* independence unless it is calibrated by a resampling scheme, and why it is better suited to ranking candidate predictors than to deciding whether dependence exists at all. Second, mutual information has no upper bound for continuous variables and no natural scale, so a value of $1.9$ is not "on the same ruler" as a correlation of $0.99$.
...
4. **MLE** can be understood as KL-divergence minimization between the empirical distribution and a parametric model.
...
This matters because model comparisons based on KL divergence or expected log-likelihood should not depend on whether we work with levels, growth rates, or another one-to-one transformation of the same quantity.
