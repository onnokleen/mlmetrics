...
The idea is to quantify the **uncertainty** or **information content** of a random variable $X$. As a first intuition, think of entropy $\mathbb{H}$ as a **generalized variance** that takes into account the entire shape of the distribution of $X$, including the tails. The analogy has one limit worth flagging immediately: variance depends on the values $X$ takes, whereas discrete entropy depends only on the probabilities. Relabeling the outcomes $\{1,2,3\}$ as $\{1,100,10000\}$ leaves the entropy unchanged while changing the variance enormously.
...
- **Not invariant under one-to-one transformations**: for differentiable, strictly monotone $g$, the change of variables $Y = g(X)$ gives $h(Y) = h(X) + \mathbb{E}[\log |g'(X)|]$
...
Also called **Kullback-Leibler (KL) divergence**. Note: it's not a metric since it's asymmetric and violates the triangle inequality. As with cross-entropy, the divergence is finite only if $q(x)=0$ implies $p(x)=0$ (i.e. $p$ is absolutely continuous w.r.t. $q$); otherwise $D_{\text{KL}}(p\parallel q)=+\infty$.
...
**Setup**: In @fig-kl-gaussian-visual we fix the reference distribution at $p = \mathcal{N}(0,1)$ and measure how much a comparison distribution $q$ differs from it using $D_{\text{KL}}(p \parallel q)$.
...
- The curve is **not symmetric** around the reference variance: understating the variance is penalized more heavily than overstating it by the same amount. This is a separate phenomenon from the asymmetry of $D_{\text{KL}}$ in its two arguments.
...
$$
where $\mathcal{F}_{t-1}$ denotes the information available at the end of period $t-1$. The sample average negative log-likelihood is therefore an average of *conditional* log-scores, not of marginal ones, and the population object it targets is the expected conditional KL divergence
$$
\bar{D}(\theta)
= \mathbb{E}\Big[D_{\text{KL}}\big(p_0(\cdot \mid \mathcal{F}_{t-1}) \,\big\|\, p_\theta(\cdot \mid \mathcal{F}_{t-1})\big)\Big],
$$
with the outer expectation taken over the distribution of the conditioning information. This is the precise sense in which a GARCH quasi-likelihood "fits the conditional variance": it minimizes an average divergence between one-step-ahead conditional distributions. One feature carries over unchanged — $\bar{D}(\theta) \geq 0$, with equality only if the model's conditional law matches the true one — and one does not: the law of large numbers that turns the sample average into $\bar{D}(\theta)$ now requires stationarity and ergodicity rather than independence.
...
Prove this property. Specifically, let $X$ be a continuous random variable with two possible probability densities, $p_X(x)$ and $q_X(x)$. Let $y = g(x)$ be a continuously differentiable, strictly monotone transformation, with $Y = g(X)$ having densities $p_Y(y)$ and $q_Y(y)$. Show that the KL divergence is unchanged:
$$
