...
$$
with the outer expectation taken over the distribution of the conditioning information. Under a Gaussian quasi-likelihood with a correctly specified conditional mean, the candidate Gaussian law is indexed by its conditional variance, and minimizing this average divergence selects the conditional second-moment projection. This is the precise sense in which a generalized autoregressive conditional heteroskedasticity (GARCH) Gaussian quasi-likelihood "fits the conditional variance." Without those qualifications, the divergence statement concerns the full conditional law and should not be read as a variance-only result. One feature carries over unchanged — $\bar{D}(\theta) \geq 0$, with equality only if the model's conditional law matches the true one — and one does not: the law of large numbers that turns the sample average into $\bar{D}(\theta)$ now requires stationarity and ergodicity rather than independence.
...
:::{.callout-warning title="Do Not Generalize the Area Diagram Mechanically"}

Treat @fig-entropy-venn as a mnemonic for the two-variable identities shown here, not as a proof or a template for arbitrary collections of variables. With three or more variables, use the algebraic definitions and chain rules rather than assuming that every information quantity must correspond to a literal nonnegative area.
:::
...
$$
Thus the entropy levels depend on the measurement unit, while the entropy difference between these two forecasts — equivalently, their relative dispersion — is invariant to a common linear rescaling.
:::
