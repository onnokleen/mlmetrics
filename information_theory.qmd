...
The last two points are specific to the continuous case. Discrete entropy is nonnegative and invariant to relabeling of the support, while differential entropy depends on the measurement scale. For example, measuring the same variable in dollars rather than thousands of dollars changes $h(X)$ by an additive term.
...
Therefore, equality holds if and only if $\frac{q(x)}{p(x)} = 1$ on the support of $p$, that is, $p(x) = q(x)$ for all $x \in \mathcal{X}$ with $p(x) > 0$.
