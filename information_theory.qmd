...
- penalized estimation and model selection, recurring themes from [the next chapter](cross_validation.qmd) onwards
...
:::{.callout-note}
**References**: @Murphy2022PML1, Chapter 6.1-6.1.4 and 6.2-6.3.5, gives a machine-learning treatment. @CoverThomas2006 is the canonical information-theory reference for the underlying entropy and divergence results.
:::
...
$$I(x) := -\log p(x).$$
Rare outcomes (small $p(x)$) carry large $I(x)$, matching the "surprise" language of the Overview. (Do not confuse the self-information $I(x)$ — a function of one outcome — with the mutual information $\mathbb{I}(X;Y)$ between two random variables, introduced at the end of this chapter.) Entropy is then the expected surprisal,
...
Suppose two density forecasts for inflation have the same mean and variance, but one assigns substantially more probability to tail events. Which quantities from this chapter are *capable* of distinguishing the two forecasts, and what comparison object does each require — a reference distribution, realized outcomes, or nothing beyond the forecast itself? Why do comparisons of means and variances fail by construction?
:::

:::{.callout-tip collapse="true" title="Suggested Answer"}

Mean and variance comparisons fail by construction, since the two forecasts share both moments. Entropy requires nothing beyond each forecast itself and *can* separate them, because it depends on the entire shape of the density — though two different shapes can happen to share the same entropy, so a difference is possible rather than guaranteed. Cross-entropy can distinguish them once a reference distribution or realized outcomes are supplied: it evaluates the log density across the support, so tail mass matters directly — against a fat-tailed truth, a forecast with too little tail mass has a high expected negative log-likelihood, and the divergence measure introduced in the next section registers the same gap. The general lesson: what a quantity can detect depends on what it is fed — the forecast alone, or the forecast against a reference.
:::
...
With equal variances ($\sigma_0^2=\sigma_1^2$), the two directions coincide for any pair of means; once $\sigma_0^2 \neq \sigma_1^2$, the divergence is asymmetric in general. Direction matters in optimization and approximation tasks.
...
$$= \underbrace{\mathbb{E}_{X \sim \hat{p}_{\text{data}}}[\log \hat{p}_{\text{data}}(X)]}_{\text{constant w.r.t. } \theta} - \underbrace{\mathbb{E}_{X \sim \hat{p}_{\text{data}}}[\log p_\theta(X)]}_{\text{average log-likelihood}}$$
...
Read this empirical version as a mnemonic rather than as the formal statement. It requires $p_\theta$ to be a probability mass function on the same discrete set as $\hat{p}_{\text{data}}$; for continuous data, $\hat{p}_{\text{data}}$ places mass on finitely many points while $p_\theta$ is a density, so the two are not mutually absolutely continuous and $D_{\text{KL}}(\hat{p}_{\text{data}} \parallel p_\theta)$ is not defined. The population statement above is the one that holds in general.

**What changes under dependence.** Everything so far assumed an i.i.d. sample, which is rarely the right model for the macroeconomic and financial series this book works with. The repair is the prediction-error decomposition: for observations $x_1,\dots,x_T$ the joint log-likelihood factorizes as
$$
...
$$
with the outer expectation taken over the distribution of the conditioning information. This is the precise sense in which a generalized autoregressive conditional heteroskedasticity (GARCH) quasi-likelihood "fits the conditional variance": it minimizes an average divergence between one-step-ahead conditional distributions. One feature carries over unchanged — $\bar{D}(\theta) \geq 0$, with equality only if the model's conditional law matches the true one — and one does not: the law of large numbers that turns the sample average into $\bar{D}(\theta)$ now requires stationarity and ergodicity rather than independence.
...
Read the left panel as follows. The blue circle is all the uncertainty in $X$ and the orange circle all the uncertainty in $Y$. The region they share is the information the two variables hold in common, which the next section defines formally as the mutual information $\mathbb{I}(X;Y)$. What remains of the blue circle after removing that overlap is the uncertainty about $X$ that survives once $Y$ is known, namely $\mathbb{H}(X \mid Y)$, and symmetrically on the orange side. The union of the two circles is the joint entropy.
...
:::{.callout-warning}
The area metaphor is reliable for two variables and only for two. With three or more, the analogous "shared" region can be negative, so no diagram of overlapping areas represents the quantities faithfully. Treat @fig-entropy-venn as a mnemonic for the two-variable identities above, not as a general proof device.
:::
...
For continuous variables the sum becomes an integral against the joint density, $\mathbb{I}(X;Y)=\int p(x,y)\log\frac{p(x,y)}{p(x)\,p(y)}\,dx\,dy$, with differential entropies replacing discrete ones in the decomposition; the properties below hold in both cases. Either way, mutual information is the KL divergence between the joint distribution and the product of its marginals — the independence benchmark — a representation proved in Exercise 1.6.
...
# --- Circular: x pins down y up to sign ---
...
Two cautions about reading the numbers. First, they are *estimates*, not population quantities. Mutual information for continuous variables has to be estimated nonparametrically [@KraskovStogbauerGrassberger2004], which is considerably harder than estimating a correlation: the independent panel returns $0.053$ rather than $0.000$ — finite-sample estimation error in this realization, kept nonnegative by the estimator's truncation at zero. Estimation error of this kind is why a raw MI estimate is a poor instrument for formally *testing* independence unless its null distribution is calibrated, for instance by a permutation scheme, and why in practice such estimates serve more often to screen candidate predictors than to decide whether dependence exists at all. Second, mutual information has no upper bound for continuous variables and no natural scale, so a value of $1.9$ is not "on the same ruler" as a correlation of $0.99$.
...
:::{.callout-warning collapse="true"}
## Hint for Part 2
For the units reconciliation, specialize the transformation rule $h(Y)=h(X)+\mathbb{E}[\log|g'(X)|]$ from the chapter to a linear rescaling $g(x)=ax$, and ask which $a$ reproduces the entropy difference.
:::
...
=\frac{1}{2}\log c,
$$
which depends only on the variance *ratio* $c$, not on the level $\sigma_1^2$: doubling the variance adds $\tfrac12\log 2$ of entropy whether the forecast was tight or diffuse to begin with.

The same difference arises from a pure change of units. Rescaling by $g(x)=\sqrt{c}\,x$ multiplies the variance by $c$, and the transformation rule gives
$$
h\big(\sqrt{c}\,X\big)=h(X)+\log\sqrt{c}=h(X)+\frac{1}{2}\log c .
$$
Entropy comparisons between Gaussian forecasts therefore measure relative dispersion — exactly the quantity a change of measurement scale alters — which is another way of seeing why only entropy *differences*, not levels, carry meaning for continuous variables.
:::
...
$$ \mathbb{E}_{X \sim p}\left[\frac{q(X)}{p(X)}\right] = \sum_{x:\, p(x)>0} p(x) \frac{q(x)}{p(x)} = \sum_{x:\, p(x)>0} q(x) \le 1 $$
Note that the expectation under $p$ sums only over the support of $p$, so it equals the $q$-probability of that support, which is at most 1, with equality exactly when $q$ places no mass outside the support of $p$.

Substituting this back into our inequality and using that $-\log$ is decreasing:
$$ D_{\text{KL}}(p \parallel q) \ge -\log\left(\sum_{x:\, p(x)>0} q(x)\right) \ge -\log(1) = 0 $$
This proves that $D_{\text{KL}}(p \parallel q) \ge 0$.

**Part 2: Characterizing Equality**

The derivation in Part 1 used two inequalities, and $D_{\text{KL}}(p \parallel q) = 0$ requires both to be tight. First, Jensen's inequality holds with equality if and only if the argument of the convex function is $p$-almost surely constant, i.e. $\frac{q(x)}{p(x)} = c$ for some constant $c$ on $\{x : p(x) > 0\}$. Second, the final step is tight if and only if $\sum_{x:\, p(x)>0} q(x) = 1$, i.e. $q$ places no mass outside the support of $p$. Combining the two: summing $q(x) = c\,p(x)$ over the support of $p$ gives $c = 1$, so $q(x) = p(x)$ on the support of $p$; and since $q$ has no mass elsewhere, $q$ and $p$ agree everywhere. Therefore $D_{\text{KL}}(p \parallel q) = 0$ if and only if $p(x) = q(x)$ for all $x \in \mathcal{X}$.
...
Prove this property. Specifically, let $X$ be a continuous random variable with two possible probability densities, $p_X(x)$ and $q_X(x)$. Let $y = g(x)$ be a continuously differentiable, strictly monotone transformation with $g'(x) \neq 0$ for all $x$, and let $Y = g(X)$ have densities $p_Y(y)$ and $q_Y(y)$. Show that the KL divergence is unchanged:
$$
...
Let $y=g(x)$ be continuously differentiable and strictly monotone with $g'(x) \neq 0$ for all $x$, so that by the inverse function theorem the inverse $x=g^{-1}(y)$ exists and is differentiable. The probability densities for the new variable $y$ are given by:
$$
...
$$
(Strictly, the integral runs over the range of $g$, which need not be all of $\mathbb{R}$ — think of $g(x)=e^x$ mapping onto $(0,\infty)$. Outside that range $p_Y$ vanishes and the integrand is zero, so the infinite bounds are harmless, and the substitution below maps the range of $g$ back onto the support of $X$.) We can now perform a change of variables in the integral itself, from $y$ back to $x$. Using $x = g^{-1}(y)$, we have $dx = \left| \frac{dx}{dy} \right| dy$. Substituting this into the integral gives:
$$
...
Let $p$ be the density of $\mathcal{N}(\mu_0,\sigma^2)$ and $q$ the density of $\mathcal{N}(\mu_1,\sigma^2)$, so the two Gaussian distributions have the same variance but different means.
...
2. Suppose two competing models, with densities $q_A$ of $\mathcal{N}(\mu_A,\sigma^2)$ and $q_B$ of $\mathcal{N}(\mu_B,\sigma^2)$, use the same variance as the true density $p$. Show that
   $$
...
$$
and consider candidate models of the same form with parameters $\theta=(\phi,\sigma^2)$, $\sigma^2>0$, so that the model's conditional density is $x_t \mid x_{t-1} \sim \mathcal{N}(\phi x_{t-1},\sigma^2)$.
...
1. Write down the factorization of the joint model density $p_\theta(x_2,\dots,x_T \mid x_1)$ into one-step-ahead conditional densities and show that the average conditional-on-$x_1$ negative log-likelihood is
   ...
   $$
2. Using the two-Gaussian KL formula from the chapter, show that for a given value of $x_{t-1}$ the conditional KL divergence is
   $$
...
Under the model, conditional on the past only the most recent lag matters, and $x_t \mid x_{t-1} \sim \mathcal{N}(\phi x_{t-1},\sigma^2)$. Hence
$$
...
For a given $x_{t-1}$, both conditional distributions are Gaussian: the truth is $\mathcal{N}(\phi_0 x_{t-1},\sigma_0^2)$ and the model is $\mathcal{N}(\phi x_{t-1},\sigma^2)$. The two-Gaussian formula with means $\phi_0 x_{t-1}$ and $\phi x_{t-1}$ and variances $\sigma_0^2$ and $\sigma^2$ gives
$$
D_{\text{KL}}\big(p_0(\cdot \mid x_{t-1})\,\big\|\,p_\theta(\cdot \mid x_{t-1})\big)
=\frac{(\phi_0-\phi)^2 x_{t-1}^2}{2\sigma^2}
+\frac{1}{2}\left(\frac{\sigma_0^2}{\sigma^2}-1+\log\frac{\sigma^2}{\sigma_0^2}\right).
...
The stationary distribution of the true process is $\mathcal{N}\big(0,\sigma_0^2/(1-\phi_0^2)\big)$, so the proposed white-noise model — $\phi=0$, $\sigma^2=\sigma_0^2/(1-\phi_0^2)$ — implies exactly the same unconditional distribution. Substituting into $\bar D(\theta)$: the first term becomes
$$
