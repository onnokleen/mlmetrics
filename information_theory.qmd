...
Let $u_t$ denote the innovation after specifying the conditional mean, let
$$
v_0(\mathcal{F}_{t-1})
=
\mathbb{E}[u_t^2\mid\mathcal{F}_{t-1}]
$$
be its true conditional variance, and let $h_\theta(\mathcal{F}_{t-1})$ be the candidate variance. Up to constants, Gaussian quasi-likelihood selects $\theta$ to minimize
$$
\mathbb{E}\left[
