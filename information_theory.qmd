...
:::{.callout-important}
## Key Takeaways
1. Entropy measures expected surprise under a distribution.
2. When finite, cross-entropy equals entropy plus KL divergence.
3. Maximum likelihood minimizes sample log loss, whose population counterpart is expected log loss.
4. Mutual information measures dependence beyond linear correlation.
:::

:::{.callout-warning}
## Common Pitfalls
- Matching means and variances does not establish that two distributions have the same shape or tails.
- Differential entropy depends on units, so comparisons across transformations require care.
- The arguments of $D_{\mathrm{KL}}(p\parallel q)$ have different roles and cannot generally be exchanged.
- Zero KL divergence implies equality of probability distributions, but continuous densities need only agree almost everywhere.
:::
