...
Let $X$ be discrete with probability mass function $p(x)$, and let $\mathcal{X}$ denote its support: the set of values $x$ with $p(x)>0$.
...
:::{.callout-note}
In classical information theory, it's convenient to use $\log_2$ instead of $\log_e$ for the interpretation as "bits." We'll stick to $\log_e$, in which case entropy is measured in *nats*.
:::
...
Maximum uncertainty occurs at $\theta = 0.5$, where the outcome is most unpredictable.
...
**Key Result**: Among all distributions with given variance on $\mathbb{R}$, the Gaussian has the largest entropy. The maximum-entropy principle — choosing the least informative distribution consistent with stated constraints — was developed by @Jaynes1957 and is a workhorse for distributional priors in Bayesian econometrics and information-theoretic estimation.

**Interpretation**: There is no uniform distribution on $\mathbb{R}$, but the Gaussian is the maximum-entropy distribution under the variance constraint — it is the least concentrated, or most spread out, among all distributions with that variance.
...
- Not invariant under one-to-one transformations: a change of variables $Y = g(X)$ adds the expected log-Jacobian $\mathbb{E}[\log |g'(X)|]$ to $h(X)$
- Can be negative (unlike discrete entropy)

The last two points are specific to the continuous case. Discrete entropy is nonnegative and invariant under any bijective relabeling of the alphabet, but it still depends on the chosen partition: coarsening or refining the support changes its value. Differential entropy depends on the measurement scale: measuring the same variable in dollars rather than thousands of dollars changes $h(X)$ by an additive constant.

These limitations motivate the relative measures introduced next.
...
However, cross-entropy still has limitations in the continuous-density case: it can be negative and is not invariant to reparameterization. In the discrete case, cross-entropy is nonnegative when probabilities are used directly. The invariant object is the log-score difference, or equivalently the KL divergence, where the transformation terms cancel.
...
For a discrete empirical distribution, the same algebra can be written as a KL divergence between the empirical distribution $\hat{p}_{\text{data}}$ — the empirical distribution placing probability $1/n$ on each of the $n$ observed data points — and the model distribution $p_\theta$:
...
This provides an information-theoretic justification for the MLE principle. In words: maximum likelihood chooses the model whose implied distribution is closest, in KL divergence, to the empirical distribution of the data. Under misspecification, the limit of the MLE is the parameter value that minimizes KL divergence to the true distribution — this is the quasi-MLE interpretation of @White1982.
:::
...
- Stating equality conditions for KL Divergence outside the support where $p(x)>0$.
:::
...
Jensen's inequality holds with equality if and only if the argument of the convex function is $p$-almost surely constant on the support of $p$. In our case, this means $\frac{q(x)}{p(x)} = c$ for some constant $c$ on $\{x : p(x) > 0\}$. Summing both sides against $p$,
$$ \sum_x q(x) = \sum_x c \cdot p(x) \implies 1 = c \sum_x p(x) \implies c=1 $$
on the support of $p$. Therefore equality holds if and only if $p(x) = q(x)$ for all $x \in \mathcal{X}$ with $p(x) > 0$. Outside the support of $p$, the value of $q$ is unconstrained by KL.
