...
- $D$ should be invariant to reparameterization; e.g., if $X$ has density $p(x)$ and we define $Y = (X-\mu)/\sigma$, the divergence between the distributions of $Y$ under two probability measures should not depend on the scale $\sigma$ or shift $\mu$
...
However, cross-entropy still has limitations in the continuous-density case: it can be negative, and it is not invariant to reparameterization. (In the discrete case it is nonnegative, since $\log q(x) \leq 0$ for probabilities.) Two objects built from cross-entropy *are* invariant, because the log-Jacobian term is common to both densities and cancels: the difference in cross-entropy between two candidate models, $h_{ce}(p,q_1) - h_{ce}(p,q_2)$, and the KL divergence introduced next. These are different quantities — the first ranks two models against each other, the second measures how far one model is from $p$ — but a change of variables leaves both unchanged.
...
Cross-entropy would also notice the difference once a reference distribution or realized outcomes are used: it evaluates the log density assigned across the support, so tail probabilities matter directly. If the true inflation distribution puts substantial probability in the tails, a forecast with too little tail mass has a high expected negative log-likelihood — and the divergence measure introduced in the next section registers the same gap. If the true distribution is thin-tailed, a tail-heavy forecast wastes probability mass in regions that are rarely realized.
:::

## Relative Entropy (KL Divergence)

We combine entropy and cross-entropy to define a divergence measure with three properties: non-negative, zero if and only if the two distributions agree on the support of the reference, and invariant under one-to-one reparameterizations.
...
1. **Non-negativity**: $D_{\text{KL}}(p\parallel q) \geq 0$, with equality if and only if $p(x) = q(x)$ for all $x$ with $p(x) > 0$ (in the continuous case: almost everywhere with respect to $p$). For a proof, see the exercises below.
...
:::{.callout-important}
## Key Takeaways
1. **Entropy** quantifies uncertainty in a distribution, while differential entropy must be interpreted carefully because it is not invariant to reparameterization.
...
- Reading $D_{\text{KL}}(p\parallel q)=0$ as "$p$ and $q$ agree everywhere" without noting that the argument pins them down only where $p(x)>0$.
:::
...
:::{.callout-warning collapse="true"}
## Hint for Part 2
For the units reconciliation, specialize the transformation rule $h(Y)=h(X)+\mathbb{E}[\log|g'(X)|]$ from the chapter to the linear map $g(x)=\sqrt{c}\,x$.
:::
...
Show that the KL divergence is invariant to a change of variables (reparameterization). 
...
2. Suppose two competing models, with densities $q_A$ of $\mathcal{N}(\mu_A,\sigma^2)$ and $q_B$ of $\mathcal{N}(\mu_B,\sigma^2)$, use the same variance as the true density $p$. Show that
   ...
   |\mu_0-\mu_A|<|\mu_0-\mu_B|,
   $$
   and conclude that in this special Gaussian case, KL-based model comparison is equivalent to comparing squared forecast bias in the conditional mean.
