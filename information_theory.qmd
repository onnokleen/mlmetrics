...
=\frac{1}{2}\log c,
$$
which depends only on the variance *ratio* $c$, not on the level $\sigma_1^2$: doubling the variance adds $\tfrac12\log 2$ whatever the starting level.
...
$$ D_{\text{KL}}(p \parallel q) = \sum_{x \in \mathcal{S}_p} p(x) \log\frac{p(x)}{q(x)}. $$
...
2. Show that $D_{\text{KL}}(p \parallel q)=0$ if and only if $p(x)=q(x)$ for all $x \in \mathcal{S}_p$.
3. Deduce from Part 1 that for every candidate distribution $q$ satisfying the standing assumption,
   $$
...
$$ D_{\text{KL}}(p \parallel q) = \sum_{x \in \mathcal{S}_p} p(x) \log\frac{p(x)}{q(x)} = - \sum_{x \in \mathcal{S}_p} p(x) \log\frac{q(x)}{p(x)} $$
...
$$ \mathbb{E}_{X \sim p}\left[\frac{q(X)}{p(X)}\right] = \sum_{x \in \mathcal{S}_p} p(x) \frac{q(x)}{p(x)} = \sum_{x \in \mathcal{S}_p} q(x) \le 1 $$
...
$$ D_{\text{KL}}(p \parallel q) \ge -\log\left(\sum_{x \in \mathcal{S}_p} q(x)\right) \ge -\log(1) = 0 $$
...
$$ D_{\text{KL}}(p \parallel q) = \mathbb{E}_{X \sim p}[\log p(X)] - \mathbb{E}_{X \sim p}[\log q(X)], $$
...
3. Let $X$ be uniform on $\{-1,0,1\}$ and $Y=X^2$. Show that $\operatorname{Cov}(X,Y)=0$, compute $\mathbb{I}(X;Y)$, and explain why this example proves that zero correlation does not imply independence.”
...
&= \mathbb{I}(X;Y),
\end{align*}
where the second line uses $\sum_{y \in \mathcal{Y}} p(x,y)=p(x)$ and $\sum_{x \in \mathcal{X}} p(x,y)=p(y)$.
If $X$ and $Y$ are independent, then $p(x,y)=p(x)p(y)$, so the KL divergence is zero and therefore
$$
\mathbb{I}(X;Y)=0.
$$

**Part 3: Zero Correlation with Positive Mutual Information**

Since $X$ is uniform on $\{-1,0,1\}$, $\mathbb{E}[X]=0$ and $\mathbb{E}[XY]=\mathbb{E}[X^3]=\tfrac13(-1+0+1)=0$, so
$$
...
$$
\mathbb{H}(Y)=-\tfrac13\log\tfrac13-\tfrac23\log\tfrac23=\log 3-\tfrac23\log 2.
$$
Hence
$$
\mathbb{I}(X;Y)=\mathbb{H}(X)+\mathbb{H}(Y)-\mathbb{H}(X,Y)=\log 3-\tfrac23\log 2\approx 0.64>0.
$$
