...
$$\mathbb{H}(X,Y) := -\sum_{x\in\mathcal{X}}\sum_{y\in\mathcal{Y}} p(x,y)\log p(x,y)$$

**Conditional entropy** (uncertainty of $Y$ given $X$):
$$\mathbb{H}(Y|X) := -\sum_{x\in\mathcal{X}}\sum_{y\in\mathcal{Y}} p(x,y)\log p(y|x)$$
...
$$\mathbb{I}(X;Y) := \sum_{x\in\mathcal{X}}\sum_{y\in\mathcal{Y}} p(x,y) \log\frac{p(x,y)}{p(x)p(y)} = \mathbb{H}(X) + \mathbb{H}(Y) - \mathbb{H}(X,Y)$$
...
1. Start from the non-negativity of KL divergence:
   $$
   D_{\mathrm{KL}}(p \Vert q) = \int_{\mathbb{R}} p(x)\log\frac{p(x)}{q(x)}\,dx \ge 0.
   $$
   This property is stated in the Properties of KL Divergence callout and proved in Exercise 1.3. Show that
   $$
...
This can be seen as the negative expectation of $\log\frac{q(X)}{p(X)}$ when $X$ is distributed according to $p$:
