...
1. Show that
   ...
   $$
2. Show that the divergence is zero if and only if $\mu_0=\mu_1$, and explain why the divergence grows quadratically in the misspecification of the mean.
3. Suppose $P$ is the true predictive density and two competing models imply means $\mu_A$ and $\mu_B$ with the same variance $\sigma^2$. Explain which model is preferred under KL divergence and why this is equivalent to choosing the model with the smaller squared mean error.
...
The divergence is zero if and only if $(\mu_0-\mu_1)^2=0$, that is, if and only if $\mu_0=\mu_1$. Because the formula is quadratic in $(\mu_0-\mu_1)$, mean misspecification is penalized more severely the further the model mean moves away from the truth.

**Part 3**

If both competing models use the same variance $\sigma^2$, then their KL divergences from the true density are
$$
\frac{(\mu_0-\mu_A)^2}{2\sigma^2}
\qquad \text{and} \qquad
\frac{(\mu_0-\mu_B)^2}{2\sigma^2}.
$$
So the preferred model is the one with the smaller squared mean error. In this special Gaussian case, minimizing KL divergence is therefore equivalent to minimizing squared forecast bias in the conditional mean.
...
2. Define the mutual information by
   ...
   $$
   Use Part 1 to show that independence implies $I(X;Y)=0$.
...
I(X;Y)=\mathbb{H}(X)+\mathbb{H}(Y)-\mathbb{H}(X,Y).
$$
If $X$ and $Y$ are independent, Part 1 shows that
$$
\mathbb{H}(X,Y)=\mathbb{H}(X)+\mathbb{H}(Y),
$$
so
