...
A related variant, often called **subagging**, instead draws subsamples of size $L<n$, usually without replacement. Both approaches create many perturbed training sets; the random forest then applies equal weights $1/B$ to the resulting tree predictions.
...
The OOB construction produces an approximately honest prediction for observation $i$ because the trees contributing to that prediction were not trained on $i$. The **OOB error** is the average loss of these OOB predictions over all observations; for regression it is
$$
\text{OOB error}
=
...
#| fig-cap: "Out-of-bag (OOB) MSE (blue) and independent-test MSE (orange) against the number of trees in a simulated regression forest."
#| echo: true
#| warning: false
