...
Common rules of thumb are to try roughly $p/3$ candidate features per split for regression and roughly $\sqrt{p}$ for classification [@Breiman2001RandomForests]. These are not econometric laws; they are starting values that should be checked with a validation design appropriate for the data structure.
