...
This weighting view is also the bridge to more advanced forests. *Quantile regression forests* keep more than the leaf mean and use the same neighborhood weights to estimate a conditional distribution [@Meinshausen2006QRF]. *Local linear forests* go in a different direction: they retain the forest neighborhood but fit a local linear model rather than a constant inside it [@FriedbergTibshiraniAtheyWager2021]. Both are developed in the next chapter, in the [quantile regression forest section](adv_tree_based_methods.qmd#sec-adv-qrf) and the [local linear forest section](adv_tree_based_methods.qmd#sec-adv-llf) respectively.
