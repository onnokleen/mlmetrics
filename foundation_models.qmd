...
$$ {#eq-fm-avgnll}
Suppose the test sequence $W_{1:T}$ is drawn from a true joint distribution $p^\star$ that is *stationary and ergodic* on the vocabulary $\mathcal{V}$. Then, as $T \to \infty$, a generalized Shannon–McMillan–Breiman argument [@Barron1985; @AlgoetCover1988] gives
$$
...
$$ {#eq-fm-crossent}
Two regularity points deserve care here. First, the convergence result needs conditions on the *model* as well as on the data: the cited theorems cover the case where $p_\theta(\cdot \mid \cdot)$ is (eventually) a time-invariant function of a fixed number of lagged tokens — a Markov structure that a transformer with a finite context window satisfies at positions beyond the window length. Second, the per-position expectation $-\mathbb{E}_{p^\star}[\log p_\theta(W_t \mid W_{1:t-1})]$ need not be free of $t$ even when $p^\star$ is stationary, because the conditioning history grows with $t$; it stabilizes once the model's effective conditioning window is full, which is why the rate in @eq-fm-crossent is defined as a limit of averages rather than as a single per-position expectation. With these caveats, the limit is exactly the cross-entropy object studied in [Information Theory](information_theory.qmd). **Perplexity** is defined as
$$
\text{PPL}(w_{1:T}; \theta) \;=\; \exp\!\bigl(\bar{\ell}(w_{1:T}; \theta)\bigr).
$$ {#eq-fm-ppl}
Perplexity has the interpretation of an *effective vocabulary size per token* in the following specific sense: if at every position $t$ the conditional distribution $p_\theta(\,\cdot\, \mid w_{1:t-1})$ is uniform on some subset of $K$ tokens, then $\bar{\ell} = \log K$ and $\text{PPL} = K$. In general $\text{PPL}$ is the geometric mean of the reciprocals of the conditional probabilities assigned to the observed tokens. A lower perplexity means the model assigns, on average, a higher probability to the tokens that occur, and is better in a predictive-distribution sense in the terminology of [Evaluating Predictive Distributions](evaluating_distributions.qmd).
...
**Threshold effects in classification.** Asking a model to return a discrete class and then encoding that class as a number (for example in $\{-1, 0, +1\}$) discards calibration information and violates the conditional-mean condition in @eq-fm-meas everywhere, not only near decision boundaries. For any numeric encoding of an unordered discrete label, $\mathbb{E}[z_t \mid s_t]$ is determined by the class-assignment probabilities at each value of $s_t$ and there is no reason for this conditional expectation to equal $s_t$ on any scale.
...
In practice the forecaster typically draws from several documents at each origin rather than a single one. At forecast origin $t$ the text-based procedure selects a document set $D_t$ and a *set-valued* labeling map $G$ produces a text feature
$$
x_t^{\text{text}} \;=\; G(D_t) \;\in\; \mathbb{R}^{k_z},
$$
where $G$ aggregates the per-document scores $g(d)$ (for example by averaging, weighting, or concatenating $g(d)$ across $d \in D_t$) into a vector of dimension $k_z \ge 1$. When $D_t = \{d_t\}$ is a singleton, $G(\{d_t\}) = g(d_t)$ reduces to the scalar $z_t$ of @sec-fm-measurement.
...
**Conformal wrappers.** If we want prediction intervals rather than point forecasts, the conformal machinery of [Conformal Prediction](conformal_prediction.qmd) applies, but with a warning. The finite-sample marginal coverage guarantee of split conformal prediction requires exchangeability between the calibration and test non-conformity scores, which is not automatic when the text-derived regressor $z_t$ is produced by a foundation model whose behavior drifts with model-version updates, prompt revisions, or training-cutoff changes. In time-series settings, the specialized variants discussed in [Conformal Prediction](conformal_prediction.qmd) (block-wise, adaptive, and online-calibration schemes) are the appropriate tool; the plain split-CP exchangeability argument is fragile here.
...
The retriever as specified is **not admissible**. The admissibility condition is: for each forecast origin $t$, the retriever's parameters $\psi_t$ must be $\mathcal{I}_t$-measurable, i.e., they must have been estimable from information available on or before $t$.
...
The foundation model $p_\theta$ is **not admissible** for the 2019-Q4 forecast origin because its training cutoff is 2021-09. Any knowledge about events between $t$ and 2021-09 that is encoded in $\theta$ contaminates $p_\theta$.
