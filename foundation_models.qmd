...
$$ {#eq-fm-avgnll}
Suppose the test sequence $W_{1:T}$ is drawn from a true joint distribution $p^\star$ that is *stationary and ergodic* on the vocabulary $\mathcal{V}$. Then, as $T \to \infty$, a standard ergodic-theorem argument [@CoverThomas2006, Ch. 4] gives
$$
\bar{\ell}(W_{1:T}; \theta) \;\xrightarrow{\text{a.s.}}\; H(p^\star, p_\theta),
$$
where the limit is the (stationary) cross-entropy rate
$$
H(p^\star, p_\theta) \;=\; -\,\mathbb{E}_{p^\star}\!\left[\log p_\theta(W_t \mid W_{1:t-1})\right].
$$ {#eq-fm-crossent}
Under stationarity, the right-hand side of @eq-fm-crossent does not depend on $t$. This is exactly the cross-entropy object studied in [Information Theory](information_theory.qmd). **Perplexity** is defined as
$$
...
**Part 4.** The real aggregate mean remains
$$
\mathbb{E}_F[R]=0.7\cdot 3+0.3\cdot 6=3.9.
$$
