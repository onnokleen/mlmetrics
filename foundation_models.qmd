...
**Embeddings, fixed and contextual.** A *fixed* embedding is a map $\bar e: \mathcal{V} \to \mathbb{R}^d$ that assigns to each token $w \in \mathcal{V}$ a single vector $\bar e(w)$, independent of context. A *contextual* embedding is a sequence of vectors $\phi_1, \ldots, \phi_T \in \mathbb{R}^d$, where $\phi_t$ is produced by a function of the full sequence (for causal models, of $w_{1:t}$; for non-causal models, of $w_{1:T}$). Contextual embeddings are what let the word *bank* be represented by different vectors in "central bank" and "river bank".
