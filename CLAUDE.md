...
- **Fit cache in `distributions_via_nns.qmd`**: this chapter keeps `cache: false`, because knitr cannot restore Python variables of a cached chunk without the `dill` package. Instead, a hidden helper `cached_arrays()` stores the fitted networks' outputs in `distributions_via_nns_cache/fits/` (git-ignored). The key covers the text of the fitting chunks, the Keras and TensorFlow versions, and the data arrays, so editing those chunks refits automatically (about 85 s cold, about 15 s warm). Before publishing, force a fresh fit once with `MLBOOK_REFIT=1 quarto render distributions_via_nns.qmd`. If the fitting chunks are renamed, update the label list in the `distnn-fit-fingerprint` chunk; it stops the render if any listed chunk is missing.
