...
**Daily CSV** (`SPY_daily_measures.csv`)—used by the neural-network illustration and distributional-network chapters:

| Variable | Description |
|----------|-------------|
| `date` | Trading day |
| `close_taq` | Closing price in US dollars: the last price on the day's 5-minute grid of cleaned regular-session trades. Close-to-close log returns computed from it are price returns and exclude dividends |
