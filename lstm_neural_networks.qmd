...
| **Parameters (recurrent cell)** | $H(H+p) + H$                                                    | $4H(H+p) + 4H$ (four weight blocks, $\approx 4\times$ the RNN cell)     |
...
Nothing in the estimation toolkit changes when moving from a plain RNN to an LSTM. Training minimizes a loss $L$ over windows cut from the series, gradients are computed by backpropagation through time through the unrolled cell, and the fixed-window and truncated-backpropagation choices of the [previous chapter](recurrent_neural_networks.qmd#sec-rnn-windows) apply unchanged — including the caveat that dependence beyond the window length cannot be learned. What changes is what flows through the backward pass: alongside the hidden-state path, the cell-state path contributes the gate-controlled products analyzed above.
