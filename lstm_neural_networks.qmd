...
This means gradients can flow backward through many time steps without significant attenuation, enabling the learning of long-term dependencies. The decay is never switched off entirely — $f_k < 1$ strictly, so the product still shrinks geometrically — but the network chooses the rate. That is the substantive contrast with the vanilla RNN, where the decay rate is fixed by $W_h$, which simultaneously has to serve every other function of the recursion.
...
*Exam-level as scaffolding: the six steps are mechanical substitutions into the gate equations, so this works best as a warm-up for Exercise 7.2 rather than as a standalone exam question.*
:::
