...
[An alternative nice detailed discussion of LSTMs can be found in this blog post](https://colah.github.io/posts/2015-08-Understanding-LSTMs/)

Long Short-Term Memory (LSTM) networks were designed specifically to overcome the vanishing gradient problem that plagues simple RNNs. The key innovation is the introduction of a dedicated **cell state** ($C_t$) that acts as a conveyor belt or "gradient highway," allowing information to flow through time with minimal modification.

This flow is controlled by a series of **gates**, which are small neural networks that learn to (partially) open or close, deciding what information to keep, what to discard, and what to output.

There are three main gates that regulate the information flow:

-   **Forget Gate**: Decides what information to discard from the cell state.
-   **Input Gate**: Decides what new information to store in the cell state.
-   **Output Gate**: Decides what parts of the cell state to reveal as the hidden state.

## LSTM Architecture and Notation

Long Short-Term Memory (LSTM) networks introduced by @HochreiterSchmidhuber1997 extend vanilla RNNs by introducing a gating mechanism that controls long-term information flow. Before diving into the mathematical details, let's establish the notation because LSTMs have a lot going on at the same time!
...
One of the key innovations of LSTMs is how they address the vanishing gradient problem that plagues vanilla RNNs. The solution lies in the design of the cell state and its update mechanism.
...
This creates an **additive path** for gradients to flow backward, unlike the multiplicative path in vanilla RNNs. The cell state acts as a "highway" that can carry gradient information across many time steps.
...
**Key Insight**: Unlike vanilla RNNs where gradient flow is determined by fixed weight matrices, LSTMs learn *when* to allow gradients to flow (via forget gates), providing adaptive control over the gradient highway through time. This learned control mechanism is what makes LSTMs particularly effective for sequences with complex temporal dependencies.

**LSTM vs. Standard RNN: A Summary**
...
**When to use LSTM:**
-   Tasks requiring long-term memory (e.g., asset pricing, machine translation).
-   When you have sufficient training data to learn the ~4x more parameters.
...
To see how LSTMs are used in state-of-the-art econometric research, we will review the paper "Deep Learning in Asset Pricing" by @ChenPelgerZhu2024.
...
-   **The Deep Learning Approach:** Lets a neural network learn the functional form directly from the data: $\mathbb{E}[r_{i,t+1}] = f(\text{characteristics}_{i,t})$. The network can approximate any complex, non-linear function, potentially capturing the true risk-return tradeoff more accurately.
...
The trained network $f_\theta(\cdot)$ represents the estimated **Stochastic Discount Factor (SDF)**. We want to follow the authors' arguments in their neural network setup.
...
The depth of the network allows it to learn a **hierarchy of features**. For example, the first layer might learn to combine basic accounting ratios into a "Value" factor, and a deeper layer could then learn to model the interaction between this "Value" factor and momentum signals. The network automatically engineers non-linear factors.
...
This architecture allows the model to learn a state-dependent pricing function: $\mathbb{E}[r_{i,t+1}] = f(\text{characteristics}_{i,t}, \textbf{macro\_state}_t)$.
...
-   **Key Takeaway:** The paper demonstrates that combining economic domain knowledge (no-arbitrage constraints, time-varying macro states) with modern machine learning techniques (LSTMs, GANs) yields superior results compared to using either approach in isolation.
