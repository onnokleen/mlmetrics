...
1. We begin with the architecture of feed-forward networks and interpret them as flexible nonlinear basis expansions.
...
Consider a simple econometric setup where we want to model the relationship between income $X$ and consumption $Y$. Suppose the true conditional mean is
$$
m(x)=\mathbb{E}[Y \mid X=x] = \alpha + \beta x + \gamma x^2 + \delta \sin(\pi x / \bar{x}),
\qquad x \in [0,\bar{x}].
$$
Assume we compare two approximation classes on the compact interval $[0,\bar{x}]$:

- a polynomial regression of degree $p$,
- a one-hidden-layer neural network with sigmoid activation and $N$ hidden units:
  $$
  f_N(x)=\sum_{j=1}^N w_j^{(2)} g(w_j^{(1)}x+b_j^{(1)})+b^{(2)}.
  $$

1. Explain why the universal approximation theorem implies that, for every $\varepsilon>0$, there exists a sufficiently large $N$ such that
   $$
   \sup_{x \in [0,\bar{x}]} |m(x)-f_N(x)| < \varepsilon.
   $$
   State clearly which property of $m$ is needed.
2. Show that the neural network has $3N+1$ free parameters, whereas the polynomial of degree $p$ has $p+1$ free parameters. Explain why the universal approximation theorem by itself does not tell us which model will be preferable in a finite sample.
3. Suppose the sample size is modest and the researcher mainly cares about out-of-sample forecasting rather than structural interpretation. Give one argument for why the neural network may reduce approximation bias, and one argument for why it may still perform worse out of sample than a lower-order polynomial.

*Exam level. The exercise combines approximation theory with the econometric tradeoff between flexibility and estimation risk.*
:::


::: {.callout-warning collapse="true"}
## Hint for Part 1: Universal Approximation
- The universal approximation theorem applies to continuous functions on compact sets.
- Check that $m(x)$ is continuous on $[0,\bar{x}]$.
:::

::: {.callout-warning collapse="true"}
## Hint for Part 2: Parameter Comparison
- Count one input weight, one hidden bias, and one output weight per hidden unit, plus one output bias.
- Existence of an approximating network is not the same as knowing the sample size needed to estimate it reliably.
:::

::: {.callout-warning collapse="true"}
## Hint for Part 3: Trade-offs
- Separate approximation error from estimation error.
- A richer class can fit the population regression function better but can be harder to estimate precisely in small samples.
:::
...
The universal approximation theorem states that a one-hidden-layer network with a non-polynomial activation such as the sigmoid can approximate any continuous function on a compact set arbitrarily well.

Here
$$
m(x)=\alpha + \beta x + \gamma x^2 + \delta \sin(\pi x / \bar{x})
$$
...
The key property needed is continuity of the target regression function on a bounded interval.

**Part 2**

The neural network has:

- $N$ input weights $w_j^{(1)}$
- $N$ hidden biases $b_j^{(1)}$
- $N$ output weights $w_j^{(2)}$
- one output bias $b^{(2)}$
...
A polynomial of degree $p$ has coefficients
$$
...
The universal approximation theorem does not tell us which model will be preferable in finite samples because it is an existence result, not a finite-sample risk result. It does not specify how large $N$ must be, how much data are needed to estimate the parameters stably, or whether the fitted model will generalize well out of sample.

**Part 3**

A neural network may reduce approximation bias because it can capture local nonlinearities and oscillatory features more flexibly than a low-order polynomial. In the present example, the sine term is exactly the kind of feature that a simple polynomial may approximate poorly unless the polynomial degree is increased substantially.

But the neural network may still perform worse out of sample in a modest sample because the extra flexibility raises estimation variance and makes overfitting easier. For an econometric forecaster, the relevant criterion is not whether the class is universally flexible, but whether the estimated model improves predictive risk after accounting for finite-sample noise and model selection.
:::
...
1. Derive the sample gradients $\frac{\partial L}{\partial w_2}$, $\frac{\partial L}{\partial b_2}$, $\frac{\partial L}{\partial w_1}$, and $\frac{\partial L}{\partial b_1}$ for
   ...
   L(\boldsymbol{\theta}) = \frac{1}{2n} \sum_{i=1}^n (y_i - f(x_i; \boldsymbol{\theta}))^2.
   $$
2. Write the corresponding gradient descent update equations with learning rate $\eta > 0$. Identify exactly where the sigmoid derivative enters the hidden-layer updates.
...
*Exam level. The exercise makes students derive backpropagation and then explain why neural-network estimation is a much harder optimization problem than OLS.*
:::
...
- Gradient descent subtracts $\eta$ times the gradient.
- The hidden-layer gradients must pass through the derivative of the activation function.
:::
