...
One notational convention for this chapter, fixed here once: since $L$ denotes the number of layers, we write $\mathcal{L}(\boldsymbol{\theta})$ for the sample-average loss and $\ell(y_i, f(x_i;\boldsymbol{\theta}))$ for observation $i$'s contribution — the same objects the [optimization chapter](optimization.qmd) wrote as $L(\boldsymbol{\theta})$ and $\ell_i(\boldsymbol{\theta})$.
