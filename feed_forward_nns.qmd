...
A twice-differentiable function is **convex** if and only if its Hessian is positive semidefinite (PSD) at every point in its domain. Neural-network loss functions are nonlinear in the parameters (because of the activation function $g$), so the loss is in general not a convex function of $\boldsymbol \theta$. The cross-partial below provides one diagnostic: it can take either sign depending on the data and the parameter region, which is incompatible with PSD-everywhere.
