...
The factor of $\frac{1}{2}$ is sometimes included for convenience when taking derivatives. We can employ other loss functions as well, to which we return when discussing [distributional networks](distributions_via_nns.qmd).
...
Here, we penalize weights becoming too large in Frobenius norm (the Euclidean norm of all matrix entries stacked into one vector).
