...
$$\delta_i^{(L)} =  \frac{\partial \mathcal{L}}{\partial a_i^{(L)}} \frac{\partial a_i^{(L)}}{\partial z_i^{(L)}} = \frac{\partial \mathcal{L}}{\partial a_i^{(L)}} \cdot g'^{(L)}(z_i^{(L)})$$

One scope condition deserves emphasis: this formula assumes the output activation $g^{(L)}$ acts element-wise, so that $a_i^{(L)}$ depends only on its own pre-activation $z_i^{(L)}$. The softmax is the notable exception: Every output coordinate depends on every pre-activation, so the single derivative factor must be replaced by a sum over the full Jacobian, $\delta_i^{(L)} = \sum_k \frac{\partial \mathcal{L}}{\partial a_k^{(L)}} \frac{\partial a_k^{(L)}}{\partial z_i^{(L)}}$. For softmax combined with the cross-entropy loss, this sum collapses to the remarkably simple $\delta_i^{(L)} = \hat{p}_i - y_i$ (up to the $1/n$ from the loss average) — fitted class probability minus one-hot indicator, the same residual form that Exercise 5.3 derives for the sigmoid-plus-Bernoulli case.
...
$$\delta_i^{(l)} = \frac{\partial \mathcal{L}}{\partial z_i^{(l)}} = \frac{\partial \mathcal{L}}{\partial a_i^{(l)}} \frac{\partial a_i^{(l)}}{\partial z_i^{(l)}} = \frac{\partial \mathcal{L}}{\partial a_i^{(l)}} \cdot g'^{(l)}(z_i^{(l)})$$


**Step 2**: Now, activation $a_i^{(l)}$ affects all pre-activations in layer $l+1$ so the multivariate chain rule gives us
$$\frac{\partial \mathcal{L}}{\partial a_i^{(l)}} = \sum_{j=1}^{H^{(l+1)}} \frac{\partial \mathcal{L}}{\partial z_j^{(l+1)}} \frac{\partial z_j^{(l+1)}}{\partial a_i^{(l)}} = \sum_{j=1}^{H^{(l+1)}} \delta_j^{(l+1)} W_{ji}^{(l+1)} = \sum_{j=1}^{H^{(l+1)}} W_{ji}^{(l+1)} \delta_j^{(l+1)} $$
...
2. For $l = L-1, \dots, 1$: 
   ...
   $$ \frac{\partial \mathcal{L}}{\partial W_{ij}^{(l)}} = \delta_i^{(l)} \cdot a_j^{(l-1)} \text{ and } \frac{\partial \mathcal{L}}{\partial b_i^{(l)}} = \delta_i^{(l)} $$
