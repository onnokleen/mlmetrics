...
which maps any real-valued score $z$ monotonically into the interval $(0,1)$. We can interpret $\hat p=\sigma(z)$ as a fitted probability and classify an observation as Class 1 when $\hat p>0.5$. Because $\sigma(0)=0.5$, this classification rule has decision boundary $z=\mathbf{x}'\mathbf{w}+b=0$. With a fixed link $g$, the perceptron is precisely a generalized linear model (GLM)-type index specification; if the link were instead left unknown and estimated, it would be a semiparametric single-index model — a *generalization* of the GLM family. Composing multiple such neurons is what distinguishes a neural network from the GLM baseline.

@fig-perceptron-threshold illustrates this rule for two predictors using $\mathbf{w}=(-0.2,1)'$ and $b=-1.4$. The dashed line collects the predictor combinations for which the score is zero.

```{python}
#| label: fig-perceptron-threshold
#| fig-cap: "A sigmoid perceptron with weights $\\mathbf{w}=(-0.2,1)'$ and bias $b=-1.4$. The dashed line is the zero-score decision boundary $x_2 = 1.4 + 0.2x_1$, where the fitted probability equals $0.5$. Class 1 points lie above the boundary and Class 0 points below it."
#| echo: true
...
- $\mathbf{W}^{(l)}$ is the weight matrix for layer $l$, with dimensions $H^{(l)} \times H^{(l-1)}$; the entry $W_{ij}^{(l)}$ connects neuron $j$ in layer $l-1$ to neuron $i$ in layer $l$, where $H^{(l)}$ is the number of units (width) of layer $l$, with $H^{(0)}=p$ the input dimension
...
The diagram in @fig-perceptron-threshold shows one unit. @fig-ffn-architecture shows what changes in a multilayer network: instead of specifying a basis expansion by hand, we let the model learn many intermediate transformed regressors $a_j^{(l)}$ and then combine them again in later layers.
...
3. **Rectified Linear Unit (ReLU)**: $\text{ReLU}(z) = \max(0, z)$
   ...
   - Use case: Hidden layers in deep networks; output layer for non-negative regression when smoothness at zero is not required (otherwise use softplus)
...
5. **Softmax**: $\text{Softmax}(\mathbf{z})_i = \frac{e^{z_i}}{\sum_{j=1}^K e^{z_j}}$ for $i = 1, \ldots, K$
   ...
   - Derivative: $\frac{\partial \text{Softmax}(\mathbf{z})_i}{\partial z_j} = \text{Softmax}(\mathbf{z})_i(\delta_{ij} - \text{Softmax}(\mathbf{z})_j)$, where $\delta_{ij}=1$ if $i=j$ and $0$ otherwise (the Kronecker delta), which is distinct from the backprop error signal $\delta_i^{(l)}$ introduced later
...
**Theorem [@Cybenko1989; @Hornik1991]**: Suppose $g$ is a nonconstant, bounded, continuous activation function, such as a sigmoid. For any continuous function $f$ on a compact (i.e., bounded and closed) set $K$ and any $\varepsilon > 0$, there exist a positive integer $H$ and parameters $\{w_j^{(1)}, b_j^{(1)}, w_j^{(2)}\}_{j=1}^{H}$ and $b^{(2)}$ such that the function
...
The notation here matches the single-hidden-layer representation introduced above. The same result extends from scalar $x$ to vector-valued $\mathbf{x} \in \mathbb{R}^p$.

The activation condition matters. The Cybenko-Hornik statement quoted here is the classical bounded-activation theorem, with sigmoids as the leading example. Modern networks often use ReLU, which is unbounded and therefore not covered by this version of the theorem. The classical extension that does cover ReLU is @LeshnoLinPinkusSchocken1993, who show that a single-hidden-layer network is a universal approximator on compact sets if and only if the activation is not a polynomial; this class includes ReLU and its variants. The practical lesson is the same -- width can approximate rich nonlinear functions on compact domains -- but the theorem being invoked is not literally the same theorem for every activation family.
...
With a fixed architecture, the network is a finite-dimensional nonlinear regression model. If the width $H_n$ is instead allowed to grow with the sample size under suitable complexity restrictions, the resulting sequence of network classes can be viewed as a **sieve** in the econometric sense: increasing width reduces approximation bias while raising estimation uncertainty — the formal frame behind the bias–variance discussion in Exercise 5.1. Universal approximation establishes that rich functions *can be approximated*; consistency of the estimated network additionally requires the complexity to grow slowly enough relative to the available data [@White1990; @Chen2007].
...
The factor of $\frac{1}{2}$ is sometimes included for convenience when taking derivatives. For a fixed network architecture, minimizing this criterion is nonlinear least squares: the regression function is nonlinear in $\boldsymbol{\theta}$, but the estimator still minimizes a sum of squared residuals. We can employ other loss functions as well, to which we return when discussing [distributional networks](distributions_via_nns.qmd).

**Cross-Entropy Loss (Classification)**

For binary classification problems, we typically use the cross-entropy (log-likelihood) loss. Let $s_i=f(x_i;\boldsymbol{\theta})$ denote the network's final real-valued score before the output sigmoid. If $y_i \in \{0, 1\}$ are the true class labels and $\hat{p}_i=\sigma(s_i)$ are the predicted probabilities, where $\sigma$ is the sigmoid function defined above, then
...
A twice-differentiable function on an open convex domain is **convex** if and only if its Hessian is positive semidefinite (PSD) at every point of that domain. Neural-network loss functions are nonlinear in the parameters (because of the activation function $g$), and the loss is in general not a convex function of $\boldsymbol \theta$. A clean way to see this is a symmetry argument. For the sigmoid, $g(-z) = 1 - g(z)$, so the parameter vectors $(w_1, b_1, w_2, b_2)$ and $(-w_1, -b_1, -w_2, b_2 + w_2)$ implement *exactly the same function* $f(x) = w_2\, g(w_1 x + b_1) + b_2$ and attain exactly the same loss. If the loss were convex, its set of minimizers would be convex, so the midpoint of a minimizer and its mirror image — which has $w_1 = b_1 = w_2 = 0$ and therefore represents a *constant* function — would also be a minimizer. Unless the best attainable fit is a constant, that is false, so the loss cannot be convex. The same symmetry shows that minimizers of neural-network losses are never unique.
...
with $a_i = g(w_1 x_i + b_1)$. This expression can be positive or negative depending on the data and parameter values. On its own, a sign-varying cross-partial does not disprove convexity — off-diagonal entries of a PSD Hessian can take either sign — but it illustrates how parameters interact across layers, creating the curvature structure that the symmetry argument above exploits; Exercise 5.2 works through these derivatives in detail.

**Connection to Econometric Concepts**

From an econometric perspective, we can view a neural network as a **highly flexible regression function**: we model the conditional mean as

$$\mathbb{E}[Y|\mathbf{X} = \mathbf{x}] \approx f(\mathbf{x}; \boldsymbol{\theta}),$$

with equality holding only under correct specification.
...
- **Stochastic gradient descent (SGD)**: Randomly sample one data point (noisy but fast)
...
We do not discuss all of these methods in detail in this book, but it is useful to recognize the terminology and the role each method plays.

- **$L_2$ Penalty (Ridge-Type Regularization):** $$\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{original}} + \lambda \sum_{l=1}^{L} ||\mathbf{W}^{(l)}||_F^2$$
Here, we penalize weights becoming too large in Frobenius norm (the Euclidean norm of all matrix entries stacked into one vector). An $L_2$ penalty on the network weights is the nonlinear-model analogue of ridge-type regularization [@HoerlKennard1970]: it shrinks parameters toward zero, although — unlike in ridge regression — the estimator remains nonlinear and nonconvex in the parameters, and penalties commonly exclude the biases. The related but distinct mechanism of **weight decay** shrinks the weights directly in the optimizer's update step, which is not always identical to adding an $L_2$ term to the loss (recall AdamW from the [optimization chapter](optimization.qmd)).
...
1. Show that $f(x)$ is piecewise linear. More precisely, show that on each open interval $(c_r,c_{r+1})$ its slope is
   $$
...
1. Derive the sample gradients $\frac{\partial \mathcal{L}}{\partial w_2}$, $\frac{\partial \mathcal{L}}{\partial b_2}$, $\frac{\partial \mathcal{L}}{\partial w_1}$, and $\frac{\partial \mathcal{L}}{\partial b_1}$ for
   ...
   \mathcal{L}(\boldsymbol{\theta}) = \frac{1}{2n} \sum_{i=1}^n (y_i - f(x_i; \boldsymbol{\theta}))^2.
   $$
   Then state the gradient descent update for the full parameter vector $\boldsymbol{\theta}$ in one line, with learning rate $\eta>0$.
...
1. Show that the negative Bernoulli log-likelihood contribution of observation $i$ is
   ...
   $$
   and explain why the sample-average loss $\mathcal{L}(\boldsymbol{\theta})=\frac{1}{n}\sum_{i=1}^n \ell_i(\boldsymbol{\theta})$ is exactly the binary cross-entropy loss.
...
Under conditional independence, the Bernoulli likelihood of observation $i$ is $p_i^{y_i}(1-p_i)^{1-y_i}$. Taking logs and negating gives the observation-level negative log-likelihood
$$
\ell_i(\boldsymbol{\theta})
=
-\left[y_i \log p_i + (1-y_i)\log(1-p_i)\right].
...
$$
which is exactly the binary cross-entropy loss displayed in the chapter.
