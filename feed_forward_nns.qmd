...
- **One layer neural network of $N$ hidden units**: 
  ...
  - Parameters: $N$ input weights, $N$ hidden-layer biases, $N$ output weights, and one output bias, for a total of $3N + 1$ parameters
- **Polynomial degree $p$**: $p+1$ parameters

The Universal Approximation Theorem guarantees that a neural network *can* approximate any continuous function arbitrarily well, but it does **not** specify how many hidden units are needed. This means that the theorem is non-constructive: it proves existence but does not tell us the optimal architecture. Without specifying a target approximation accuracy through $\varepsilon$, we cannot conclude which method should be preferred in a particular application.
