...
**Start With One Activated Affine Score.** The easiest way to remove the mystery is to begin with a single unit. A perceptron takes predictors $\mathbf{x}$, forms the affine score $\mathbf{x}'\mathbf{w}+b$ (a weighted linear combination plus an intercept), and then applies an activation function $g$:

$$
\hat y = g(\mathbf{x}'\mathbf{w}+b).
$$

If $g$ is the identity, this is just a linear regression-style mapping. If $g$ is a sigmoid, the same affine score becomes a probability-like output for binary classification. In econometric language, the perceptron is not yet "deep learning"; it is a familiar index model plus a nonlinear link.
