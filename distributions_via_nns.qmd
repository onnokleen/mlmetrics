...
The averages and PITs in @fig-distnn-empirical-summary tell the second, more cautionary half of the story. Despite the visible time variation in the predictive variance, the distributional HAR-FNN does not beat the homoskedastic baseline on either LogS or CRPS — the two scores are within rounding of each other, with the distributional model marginally worse. The PIT histograms are also broadly similar: both are roughly flat, with the distributional FNN reducing the mild edge mass at the price of a slightly bumpier interior.
...
The pedagogical lesson is methodological. A practitioner who reads only the score table without inspecting the predictive bands or the PIT histograms might conclude that the variance head has learned nothing — in fact, it has learned a sensible conditional scale. A practitioner who reads only the predictive band might conclude that the distributional model is clearly better — in fact, the score and PIT diagnostics show no improvement. Only the combination of @fig-distnn-empirical-bands and @fig-distnn-empirical-summary reveals what the model actually does and what it actually achieves on the proper-scoring-rule metrics that motivate distributional forecasting in the first place.
