...
The score and the model family play different roles. A *realized* LogS is local: it evaluates only the density assigned to the outcome that occurred. Its *expected* value is strictly proper and is uniquely minimized by the true density when that density belongs to the admissible class and the expectations are finite. A Gaussian network can adjust only its represented conditional mean and variance functions, so its population target under misspecification is the closest represented Gaussian in KL divergence. Student-$t$ and mixture outputs enlarge the family of distributions to which the expected score can respond.
:::
...
For distributions that are skewed, multimodal, or heavier-tailed over empirically relevant ranges than a single Gaussian, one parametric component can be too restrictive. **Mixture Density Networks (MDNs)**, proposed by @Bishop1994MDN, address this by modeling the conditional distribution as a mixture of simpler ones, such as Gaussians:
$$
...
A different route to flexible distributional output is to train separate network heads with the pinball loss at several quantile levels. Unless the architecture or loss imposes an ordering restriction, those estimated quantiles can cross. A parametric density head avoids crossing because all quantiles come from one cumulative distribution function.
...
The realized-variance illustration of the [previous chapter](nn_example.qmd) produced point forecasts of daily $\log RV_{t+1}$ for SPY, the S&P 500 exchange-traded fund, and evaluated them with squared and absolute losses. A Gaussian predictive distribution also requires a conditional variance. We therefore compare two such distributions using the same SPY data, log-transformed heterogeneous autoregressive (HAR) features, and final 20% test block. Because all hyperparameters are fixed in advance, this chapter combines the previous chapter's training and validation blocks into one estimation sample.[^distnn-data]
...
$$
Thus, the back-transformed predictive variance is $[c+\operatorname{softplus}(a_\phi(\mathbf{x}_t))]s_y^2$, exactly as implemented. This identity is specific to linear scaling. If the target were instead modeled on the log scale, a Gaussian predictive density for $\log RV$ would imply a lognormal density for $RV$ through a change of variables, and its level-scale mean would be $\mathbb{E}[RV]=\exp(\mu+\sigma^2/2)$. The present application stays on the log-$RV$ scale throughout. Under the scores considered here, consistent linear rescaling preserves model rankings [@Kleen2024], although absolute numerical score levels generally change. Forecasts and outcomes must therefore be placed on one common scale before their scores are compared.
