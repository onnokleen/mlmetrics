...
#| fig-cap: "Average LogS and CRPS on the test block (left panel; solid bars are LogS, hatched bars CRPS), and PIT histograms for the two forecasts (middle and right panels). LogS and CRPS are measured in different units, so bar heights are comparable only within a score, across the two models — not between the two scores. A well-calibrated forecast yields a flat PIT histogram close to the uniform density (red dashed line)."
#| echo: true
