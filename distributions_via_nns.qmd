...
Because the network is trained on the standardized target $\tilde y_t = (y_t-\bar y)/s_y$, the head $\eta_\phi$ predicts the log of the conditional variance of $\tilde y$, so the conditional variance of $y$ is $\exp(\eta_\phi(\mathbf{x}_t))\cdot s_y^2$. Without this rescaling, the predictive density would be on the wrong scale and the LogS comparison would not be meaningful.
