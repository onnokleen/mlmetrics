...
3. Analyze what happens when we add a sparsity constraint $\lambda \sum_{j=1}^{k} |h_j|$ to the loss function, where $\mathbf{h} = f_\theta(\mathbf{x})$ is the latent representation.
...
- The $L_1$ penalty $\lambda \sum_{j=1}^{k} |h_j|$ encourages sparsity (many zeros) in the latent representation
...
$$L_{sparse} = \mathbb{E}\left[\|\mathbf{x} - g_\phi(f_\theta(\mathbf{x}))\|^2\right] + \lambda \mathbb{E}\left[\sum_{j=1}^{k} |h_j|\right]$$
